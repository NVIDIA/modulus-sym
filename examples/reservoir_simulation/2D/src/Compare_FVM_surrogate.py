...
def process_chunk(chunk):
    ...
    with lock:
        ...
        progressBar = "\rPlotting Progress: " + ProgressBar(Runs, processed_chunks.value, Runs)
        ShowBar(progressBar)
        #time.sleep(1)
...
def process_step(kk):
    ...
    Plot_Modulus(ax1,nx,ny,nz,look,N_injw,N_pr,
    'pressure Modulus',injectors,producers)            
    ax2 = f_3.add_subplot(332, projection='3d')
    Plot_Modulus(ax2,nx,ny,nz,lookf,N_injw,N_pr,
    'pressure Numerical',injectors,producers)                        
    ax3 = f_3.add_subplot(333, projection='3d')
    Plot_Modulus(ax3,nx,ny,nz,diff1,N_injw,
    N_pr,'pressure diff',injectors,producers)         
    ...
    Plot_Modulus(ax1,nx,ny,nz,look,N_injw,N_pr,
    'water Modulus',injectors,producers)            
    ax2 = f_3.add_subplot(335, projection='3d')
    Plot_Modulus(ax2,nx,ny,nz,lookf,N_injw,N_pr,
    'water Numerical',injectors,producers)                        
    ax3 = f_3.add_subplot(336, projection='3d')
    Plot_Modulus(ax3,nx,ny,nz,diff1,N_injw,
    N_pr,'water diff',injectors,producers)         
    ...
    Plot_Modulus(ax1,nx,ny,nz,look,N_injw,N_pr,
    'oil Modulus',injectors,producers)            
    ax2 = f_3.add_subplot(338, projection='3d')
    Plot_Modulus(ax2,nx,ny,nz,lookf,N_injw,N_pr,
    'oil Numerical',injectors,producers)                        
    ax3 = f_3.add_subplot(339, projection='3d')
    Plot_Modulus(ax3,nx,ny,nz,diff1,N_injw,
    N_pr,'oil diff',injectors,producers)         
...
PB = P1
mpor, hpor = plan["custom"]["NVRS"]["mpor"], plan["custom"]["NVRS"]["hpor"] #minimum and maximum porosity
...
# training
LUB, HUB = plan["custom"]["NVRS"]["LUB"],plan["custom"]["NVRS"]["HUB"] # Permeability rescale
aay, bby = plan["custom"]["NVRS"]["aay"],plan["custom"]["NVRS"]["bby"] # Permeability range mD
Low_K, High_K = aay,bby


batch_size = plan["custom"]["NVRS"]["batch_size"] #'size of simulated labelled dtaa to run'
timmee =  plan["custom"]["NVRS"]["timmee"]#float(input ('Enter the time step interval duration for simulation (days): '))
max_t = plan["custom"]["NVRS"]["max_t"]#float(input ('Enter the maximum time in days for simulation(days): '))
...
if surrogate ==1:
    ...
    if (bb==False): 
        print('....Downloading Please hold.........')  
        download_file_from_google_drive('14njayJd77pKUeuCG9Jno03MbS8SaAVbn',\
    ...
    if (bba==False):  
        print('....Downloading Please hold.........')               
        download_file_from_google_drive('1fI1whx3CJSsICcvMddF2uF1hlMekWUrF',\
    ...
    if (bb==False):      
        print('....Downloading Please hold.........')      
        download_file_from_google_drive('1lmtvM8LucUU3DqSgUe7hYyYZvYS4yLnf',\
    ...
    if (bba==False):      
        print('....Downloading Please hold.........')     
        download_file_from_google_drive('10n0NgdgskQbNyTtZNG53IhttB8WCpzli',\
    ...
    if (bb==False): 
        print('....Downloading Please hold.........')  
        download_file_from_google_drive('1JGV9vNYIRUWS1Gd6bqDcG_jRbO8wEkzX',\
    ...
    if (bba==False):  
        print('....Downloading Please hold.........')               
        download_file_from_google_drive('1V_TKAqWlw3_kKaE1vnWGwWB7vlmk7t3s',\
    ...
    if (bb==False): 
        print('....Downloading Please hold.........')  
        download_file_from_google_drive('18BrbHfgM3r8OWmpuMhZi9y9geUyAI8t4',\
    ...
    if (bba==False):  
        print('....Downloading Please hold.........')               
        download_file_from_google_drive('1aSf4QYoU0ITuQdbSNRPWbtkQz7ZQh_Bt',\
...
msg = "Surrogate Reservoir simulation  took: %s secs (Wall clock time)" % timedelta(seconds=round(elapsed_time_secs))
print(msg)
print('')  
...
chunks = [list(range(i, min(i + steppi // NUM_CORES, steppi))) for i in range(0, steppi, steppi // NUM_CORES)]
...
fig4.text(0.5, 0.98, "R2(%) Accuracy - Modulus/Numerical(GPU)", ha='center', va='center', fontproperties=font, fontsize=11)
fig4.text(0.5, 0.49, "L2(%) Accuracy - Modulus/Numerical(GPU)", ha='center', va='center', fontproperties=font, fontsize=11)
...
see = Peaceman_well(inn,ouut_p,ouut_s,MAXZ,1,1e1,LUB,HUB,aay,bby,DX,steppi,\
                pini_alt,SWI,SWR,UW,BW,DZ,rwell,skin,UO,BO,pwf_producer,dt,N_inj,N_pr,nz)
    
seeTrue = Peaceman_well(inn,cPress,\
                        cSat,MAXZ,0,1e1,LUB,HUB,aay,bby,DX,steppi,\
                pini_alt,SWI,SWR,UW,BW,DZ,rwell,skin,UO,BO,pwf_producer,dt,N_inj,N_pr,nz)
