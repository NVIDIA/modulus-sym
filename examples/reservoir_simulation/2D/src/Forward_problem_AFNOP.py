...
class CustomValidatorPlotterP(ValidatorPlotter):
    ...
    def __init__(self, timmee,max_t,MAXZ,pini_alt,nx,ny,nz,steppi,tc2,dt,
                 injectors,producers,N_injw,N_pr):
        self.timmee = timmee
        self.max_t = max_t
        self.MAXZ = MAXZ
    ...
    def __call__(self, invar, true_outvar, pred_outvar):
        ...
        for itt in range(self.steppi):
            ...
            plt.subplot(1,3,1)
            Plot_2D(XX,YY,plt,self.nx,self.ny,self.nz,look,self.N_injw,self.N_pr,
            'pressure Modulus',self.injectors,self.producers)            
            plt.subplot(1,3,2)
            Plot_2D(XX,YY,plt,self.nx,self.ny,self.nz,lookf,self.N_injw,self.N_pr,
            'pressure Numerical',self.injectors,self.producers) 
            plt.subplot(1,3,3)
            Plot_2D(XX,YY,plt,self.nx,self.ny,self.nz,diff1,self.N_injw,self.N_pr,
            'pressure diff',self.injectors,self.producers) 
            ...
            Plot_Modulus(ax1,self.nx,self.ny,self.nz,look,self.N_injw,self.N_pr,
            'pressure Modulus',self.injectors,self.producers)            
            ax2 = f_3.add_subplot(132, projection='3d')
            Plot_Modulus(ax2,self.nx,self.ny,self.nz,lookf,self.N_injw,self.N_pr,
            'pressure Numerical',self.injectors,self.producers)                        
            ax3 = f_3.add_subplot(133, projection='3d')
            Plot_Modulus(ax3,self.nx,self.ny,self.nz,diff1,self.N_injw,
            self.N_pr,'pressure diff',self.injectors,self.producers) 
            
            plt.tight_layout(rect = [0,0,1,0.95])
            
            tita = '3D Map - Timestep --' + str(int((itt+1)*self.dt*self.MAXZ)) + ' days'  
        ...
        fig4.text(0.5, 0.98, "R2(%) Accuracy - Modulus/Numerical(GPU)", ha='center', va='center', fontproperties=font, fontsize=16)
        fig4.text(0.5, 0.49, "L2(%) Accuracy - Modulus/Numerical(GPU)", ha='center', va='center', fontproperties=font, fontsize=16)
        
        # Plot R2 accuracies
        plt.subplot(2,1,1)
        plt.plot(Time_vector, Accuracy_presure[:, 0], label='R2', marker='*', markerfacecolor='red', markeredgecolor='red', linewidth=0.5)
        ...
        # Plot L2 accuracies
        plt.subplot(2,1,2)
        plt.plot(Time_vector, Accuracy_presure[:, 1], label='L2', marker='*', markerfacecolor='red', markeredgecolor='red', linewidth=0.5)
        plt.title('Pressure', fontproperties=font)
        plt.xlabel('Time (days)', fontproperties=font)
...
class CustomValidatorPlotterS(ValidatorPlotter):
    ...
    def __call__(self, invar, true_outvar, pred_outvar):
        ...
        for itt in range(self.steppi):
            ...
            plt.subplot(2,3,1)
            Plot_2D(XX,YY,plt,self.nx,self.ny,self.nz,look_sat,self.N_injw,self.N_pr,
            'water Modulus',self.injectors,self.producers)            
            plt.subplot(2,3,2)
            Plot_2D(XX,YY,plt,self.nx,self.ny,self.nz,lookf_sat,self.N_injw,self.N_pr,
            'water Numerical',self.injectors,self.producers) 
            plt.subplot(2,3,3)
            Plot_2D(XX,YY,plt,self.nx,self.ny,self.nz,diff1_wat,self.N_injw,self.N_pr,
            'water diff',self.injectors,self.producers)
            R2w,L2w = compute_metrics(look_sat.ravel(), lookf_sat.ravel()) 
            Accuracy_water[itt,0] = R2w
            Accuracy_water[itt,1] = L2w              
            
            plt.subplot(2,3,4)
            Plot_2D(XX,YY,plt,self.nx,self.ny,self.nz,look_oil,self.N_injw,self.N_pr,
            'oil Modulus',self.injectors,self.producers)            
            plt.subplot(2,3,5)
            Plot_2D(XX,YY,plt,self.nx,self.ny,self.nz,lookf_oil,self.N_injw,self.N_pr,
            'oil Numerical',self.injectors,self.producers) 
            plt.subplot(2,3,6)
            Plot_2D(XX,YY,plt,self.nx,self.ny,self.nz,diff1_oil,self.N_injw,self.N_pr,
            'oil diff',self.injectors,self.producers) 
            ...
            Plot_Modulus(ax1,self.nx,self.ny,self.nz,look_sat,self.N_injw,self.N_pr,
            'water Modulus',self.injectors,self.producers)            
            ax2 = f_3.add_subplot(232, projection='3d')
            Plot_Modulus(ax2,self.nx,self.ny,self.nz,lookf_sat,self.N_injw,self.N_pr,
            'water Numerical',self.injectors,self.producers)                        
            ax3 = f_3.add_subplot(233, projection='3d')
            Plot_Modulus(ax3,self.nx,self.ny,self.nz,diff1_wat,self.N_injw,
            self.N_pr,'water diff',self.injectors,self.producers) 
            

            ax4 = f_3.add_subplot(234, projection='3d')
            Plot_Modulus(ax4,self.nx,self.ny,self.nz,look_oil,self.N_injw,self.N_pr,
            'oil Modulus',self.injectors,self.producers)            
            ax5 = f_3.add_subplot(235, projection='3d')
            Plot_Modulus(ax5,self.nx,self.ny,self.nz,lookf_oil,self.N_injw,self.N_pr,
            'oil Numerical',self.injectors,self.producers)                        
            ax6 = f_3.add_subplot(236, projection='3d')
            Plot_Modulus(ax6,self.nx,self.ny,self.nz,diff1_oil,self.N_injw,
            self.N_pr,'oil diff',self.injectors,self.producers)             
        ...
        plt.subplot(2,2,1)
        plt.plot(Time_vector, Accuracy_water[:, 0], label='R2', marker='*', markerfacecolor='red', markeredgecolor='red', linewidth=0.5)
        plt.title('water_saturation', fontproperties=font)
        plt.xlabel('Time (days)', fontproperties=font)
        plt.ylabel('R2(%)', fontproperties=font)        
        
        plt.subplot(2,2,2)
        plt.plot(Time_vector, Accuracy_oil[:, 0], label='R2', marker='*', markerfacecolor='red', markeredgecolor='red', linewidth=0.5)
        ...
        plt.subplot(2,2,3)
        plt.plot(Time_vector, Accuracy_water[:, 1], label='L2', marker='*', markerfacecolor='red', markeredgecolor='red', linewidth=0.5)
        plt.title('water_saturation', fontproperties=font)
        plt.xlabel('Time (days)', fontproperties=font)
        plt.ylabel('L2(%)', fontproperties=font) 
        ...
        plt.subplot(2,2,4)
        plt.plot(Time_vector, Accuracy_oil[:, 1], label='L2', marker='*', markerfacecolor='red', markeredgecolor='red', linewidth=0.5)
        plt.title('oil_saturation', fontproperties=font)
        plt.xlabel('Time (days)', fontproperties=font)
        plt.ylabel('L2(%)', fontproperties=font)           
...
class Black_oil(torch.nn.Module):
    ...
    def __init__(self,UIR,pini_alt,LUB, HUB,aay, bby,SWI,SWR,UW,BW,UO,BO, MAXZ,
                 nx,ny,approach):
        super().__init__()
        self.UIR = UIR
        self.UWR = UIR
        ...
        if self.approach ==1:
           ...
           # water_sat = sat
            ...
            prior_pressure = torch.zeros(sat.shape[0],sat.shape[1],\
                                self.nx,self.ny).to(device,torch.float32)
...
def run(cfg: ModulusConfig) -> None:
    ...
    timmee =  cfg.custom.NVRS.timmee#float(input ('Enter the time step interval duration for simulation (days): '))
    max_t = cfg.custom.NVRS.max_t#float(input ('Enter the maximum time in days for simulation(days): '))
    ...
    input_channel = cfg.custom.NVRS.input_channel #[Perm, Q,QW,Phi,dt, initial_pressure, initial_water_sat]    
    ...
    if (bb==False):      
        print('....Downloading Please hold.........')    
        download_file_from_google_drive('1G4Cvg8eIObyBK0eoo7iX-0hhMTnpJktj',\
    ...
    sio.savemat(to_absolute_path("../PACKETS/simulationstrain.mat"),{'perm': cPerm,\
            'Q': cQ,'Qw': cQw,\
    ...
    sio.savemat(to_absolute_path("../PACKETS/simulationstest.mat"),{'perm': cPerm,\
            'Q': cQ,'Qw': cQw,\
    'Phi': cPhi,'Time': cTime,'Pini': cPini,'Swini': cSini\
        ,'pressure': cPress,'water_sat': cSat})  
    ...
    output_keys_pressure = [
        Key("pressure", scale=(2.87008e-01, 1.85386e-01))
    ]
    
    output_keys_saturation = [
        Key("water_sat", scale=(3.12903e-01, 1.79786e-01))
    ]
    ...
    outvar_train_pressure["pressured"] = np.zeros_like(outvar_train_pressure["pressure"])
    outvar_train_saturation["saturationd"] = np.zeros_like(outvar_train_saturation["water_sat"])
    ...
    afno_pressure = AFNOArch([Key("perm", size=1),Key("Q", size=1),Key("Qw", size=1),\
    ...
    afno_saturation = AFNOArch([Key("perm", size=1),Key("Q", size=1),Key("Qw", size=1),\
    Key("Phi", size=1), Key("Time", size=1), Key("Pini", size=1),\
    ...
    darcyy = Node(
        ...
        evaluate = Black_oil(UIR,pini_alt,LUB, HUB,aay, bby,SWI,SWR,UW,BW,\
                    UO,BO, MAXZ,nx,ny,approach),
        name="Darcy node",
    )
    nodes =  [darcyy] + \
