...
class CustomValidatorPlotterP(ValidatorPlotter):
    ...
    def __call__(self, invar, true_outvar, pred_outvar):
        ...
        for itt in range(self.steppi):
            ...
            Plot_Modulus(ax1,self.nx,self.ny,self.nz,Reinvent(look),self.N_injw,self.N_pr,
            'pressure Modulus',self.injectors,self.producers)            
            ax2 = f_3.add_subplot(132, projection='3d')
            Plot_Modulus(ax2,self.nx,self.ny,self.nz,Reinvent(lookf),self.N_injw,self.N_pr,
            'pressure Numerical',self.injectors,self.producers)                        
            ax3 = f_3.add_subplot(133, projection='3d')
            Plot_Modulus(ax3,self.nx,self.ny,self.nz,Reinvent(diff1),self.N_injw,
        ...
        for i, data in enumerate([Accuracy_presure]):
            axs[0, i].plot(Time_vector, data[:, 0], label='R2', marker='*', markerfacecolor='red', markeredgecolor='red', linewidth=0.5)
        ...
        # Plot L2 accuracies
        for i, data in enumerate([Accuracy_presure]):
            axs[1, i].plot(Time_vector, data[:, 1], label='L2', marker='*', markerfacecolor='red', markeredgecolor='red', linewidth=0.5)
...
class CustomValidatorPlotterS(ValidatorPlotter):
    ...
    def __init__(self, timmee,max_t,MAXZ,pini_alt,nx,ny,wells,steppi,tc2,dt,injectors,producers,N_injw,N_pr):
        self.timmee = timmee
        self.max_t = max_t
        self.MAXZ = MAXZ
        self.pini_alt = pini_alt
    ...
    def __call__(self, invar, true_outvar, pred_outvar):
        ...
        for itt in range(self.steppi):
            ...
            Plot_Modulus(ax1,self.nx,self.ny,self.nz,Reinvent(look_sat),self.N_injw,self.N_pr,
            'water Modulus',self.injectors,self.producers)            
            ax2 = f_3.add_subplot(232, projection='3d')
            Plot_Modulus(ax2,self.nx,self.ny,self.nz,Reinvent(lookf_sat),self.N_injw,self.N_pr,
            'water Numerical',self.injectors,self.producers)                        
            ax3 = f_3.add_subplot(233, projection='3d')
            Plot_Modulus(ax3,self.nx,self.ny,self.nz,Reinvent(diff1_sat),self.N_injw,
            self.N_pr,'water diff',self.injectors,self.producers) 
            

            ax4 = f_3.add_subplot(234, projection='3d')
            Plot_Modulus(ax4,self.nx,self.ny,self.nz,Reinvent(look_oil),self.N_injw,self.N_pr,
            'water Modulus',self.injectors,self.producers)            
            ax5 = f_3.add_subplot(235, projection='3d')
            Plot_Modulus(ax5,self.nx,self.ny,self.nz,Reinvent(lookf_oil),self.N_injw,self.N_pr,
            'water Numerical',self.injectors,self.producers)                        
            ax6 = f_3.add_subplot(236, projection='3d')
            Plot_Modulus(ax6,self.nx,self.ny,self.nz,Reinvent(diff1_oil),self.N_injw,
            self.N_pr,'water diff',self.injectors,self.producers)             
        ...
        for i, data in enumerate([Accuracy_water, Accuracy_oil]):
            axs[0, i].plot(Time_vector, data[:, 0], label='R2', marker='*', markerfacecolor='red', markeredgecolor='red', linewidth=0.5)
            axs[0, i].set_title(['Water_saturation','Oil_saturation'][i], fontproperties=font)
            axs[0, i].set_xlabel('Time (days)', fontproperties=font)
            axs[0, i].set_ylabel('R2(%)', fontproperties=font)
        ...
        for i, data in enumerate([Accuracy_water, Accuracy_oil]):
            axs[1, i].plot(Time_vector, data[:, 1], label='L2', marker='*', markerfacecolor='red', markeredgecolor='red', linewidth=0.5)
            axs[1, i].set_title(['Water_saturation','Oil_saturation'][i], fontproperties=font)
            axs[1, i].set_xlabel('Time (days)', fontproperties=font)
            axs[1, i].set_ylabel('L2(%)', fontproperties=font)
        plt.tight_layout(rect=[0, 0.05, 1, 0.93])
    ...
    def forward(self, input_var: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        for m in range (sat.shape[0]): #Batch
            ...
            for i in range(self.nz):
                now = inn_now[:,i,:,:][:,None,:,:]
                dudx_fdma = dx(now, dx=dxf, channel=0, dim=0, order=1, padding="replication")
                dudy_fdma = dx(now, dx=dxf, channel=0, dim=1, order=1, padding="replication")
        ...
        for m in range (sat.shape[0]): #Batch
            ...
            for i in range(self.nz):
                now = inn_now[:,i,:,:][:,None,:,:]
                dudx_fdma = ddx(now, dx=dxf, channel=0, dim=0, order=1, padding="replication")
                dudy_fdma = ddx(now, dx=dxf, channel=0, dim=1, order=1, padding="replication")
                gulp.append(dudx_fdma)
                gulp2.append(dudy_fdma)                  
            check = torch.stack(gulp, 2)[:,0,:,:,:]
            check2 = torch.stack(gulp2, 2)[:,0,:,:]    
        ...
        for i in range(self.nz):
            inn_now2 = a1[:,:,i,:,:]
            dudx_fdma = dx(inn_now2, dx=dxf, channel=0, dim=0, order=1, padding="replication")
            dudy_fdma = dx(inn_now2, dx=dxf, channel=0, dim=1, order=1, padding="replication")
            gulp.append(dudx_fdma)
            gulp2.append(dudy_fdma)
        ...
        #Expand dcdx
        #dss = dcdx
        dsout = torch.zeros((sat.shape[0],sat.shape[1],\
                self.nz,self.nx,self.ny)).to(device,torch.float32)
        ...
        darcy_pressure = fin + (dcdx * dudx_fdm) + (a1 * dduddx_fdm) + (dcdy * dudy_fdm) + (a1 * dduddy_fdm)                 
...
def run(cfg: ModulusConfig) -> None:
    ...
    if (bb==False):      
        print('....Downloading Please hold.........')    
        download_file_from_google_drive('1PX2XFG1-elzQItvkUERJqeOerTO2kevq',\
    ...
    output_keys_pressure = [
        Key("pressure", scale=(1.16260e+00, 5.75724e-01))
    ]
    
    output_keys_saturation = [
        Key("water_sat", scale=(3.61902e-01, 1.97300e-01))
    ]
    ...
    fno_pressure = FNOArch([Key("perm", size=1), Key("Q", size=1), Key("Qw", size=1),\
             Key("Phi", size=1), Key("Time", size=1), Key("Pini", size=1),\
                Key("Swini", size=1) ],\
    fno_modes = 16, dimension=3,padding=13,nr_fno_layers=4, decoder_net=decoder1) 
    ...
    fno_saturation = FNOArch([Key("perm", size=1), Key("Q", size=1), Key("Qw", size=1),\
             Key("Phi", size=1), Key("Time", size=1), Key("Pini", size=1),\
                Key("Swini", size=1) ],\
    fno_modes = 16, dimension=3,padding=13,nr_fno_layers=4, decoder_net=decoder2)  
