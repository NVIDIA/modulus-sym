...
def Plot_performance(PINN,PINN2,trueF,nx,ny,namet,UIR,itt,dt,MAXZ,pini_alt,\
    ...
    plt.gca().set_yticks([])
    cbar1 = plt.colorbar()
    plt.clim(np.min(np.reshape(lookf[:,:,0],(-1,))),np.max(np.reshape(lookf[:,:,0],(-1,)))) 
    ...
    plt.gca().set_yticks([])
    cbar1 = plt.colorbar()
    plt.clim(np.min(np.reshape(lookf[:,:,1],(-1,))),np.max(np.reshape(lookf[:,:,1],(-1,)))) 
    cbar1.ax.set_ylabel(' Pressure (psia)',fontsize = 13)
    Add_marker(plt,XX,YY,wells) 
    ...
    plt.gca().set_yticks([])
    cbar1 = plt.colorbar()
    plt.clim(np.min(np.reshape(lookf[:,:,2],(-1,))),np.max(np.reshape(lookf[:,:,2],(-1,)))) 
    cbar1.ax.set_ylabel(' Pressure (psia)',fontsize = 13)
    Add_marker(plt,XX,YY,wells)     
    ...
    plt.gca().set_yticks([])
    cbar1 = plt.colorbar()
    plt.clim(np.min(np.reshape(lookf_sat[:,:,0],(-1,))),np.max(np.reshape(lookf_sat[:,:,0],(-1,)))) 
    ...
    plt.gca().set_yticks([])
    cbar1 = plt.colorbar()
    plt.clim(np.min(np.reshape(lookf_sat[:,:,1],(-1,))),np.max(np.reshape(lookf_sat[:,:,1],(-1,)))) 
    cbar1.ax.set_ylabel(' water_sat',fontsize = 13)
    Add_marker(plt,XX,YY,wells) 
    ...
    plt.gca().set_yticks([])
    cbar1 = plt.colorbar()
    plt.clim(np.min(np.reshape(lookf_sat[:,:,2],(-1,))),np.max(np.reshape(lookf_sat[:,:,2],(-1,)))) 
    cbar1.ax.set_ylabel(' water_sat',fontsize = 13)
    Add_marker(plt,XX,YY,wells) 
    ...
    plt.gca().set_yticks([])
    cbar1 = plt.colorbar()
    plt.clim(np.min(np.reshape(lookf_oil[:,:,0],(-1,))),np.max(np.reshape(lookf_oil[:,:,0],(-1,)))) 
    ...
    plt.gca().set_yticks([])
    cbar1 = plt.colorbar()
    plt.clim(np.min(np.reshape(lookf_oil[:,:,1],(-1,))),np.max(np.reshape(lookf_oil[:,:,1],(-1,))))  
    cbar1.ax.set_ylabel(' oil_sat',fontsize = 13)
    Add_marker(plt,XX,YY,wells)
    ...
    plt.gca().set_yticks([])
    cbar1 = plt.colorbar()
    plt.clim(np.min(np.reshape(lookf_oil[:,:,2],(-1,))),np.max(np.reshape(lookf_oil[:,:,2],(-1,)))) 
    cbar1.ax.set_ylabel(' oil_sat',fontsize = 13)
    Add_marker(plt,XX,YY,wells)      
...
def Plot_performance2(trueF,nx,ny,namet,UIR,itt,dt,MAXZ,pini_alt,\
                    steppi,wells):
...
def Peaceman_well(inn,ooutp,oouts,MAXZ,mazw,s1,LUB,HUB,aay,bby,DX,steppi,\
                pini_alt,SWI,SWR,UW,BW,DZ,rwell,skin,UO,BO,pwf_producer,dt,N_inj,N_pr,CFD,nz):  
    """
    ...
    PERM = rescale_linear_pytorch_numpy(np.reshape(PERM,(-1,),'F'),\
                                        LUB,HUB, aay, bby) 
    ...
    for kk in range(steppi):
        ...
        average_pressure = (Ptito.ravel()[producer_location] ) * pini_alt# np.mean(Ptito.ravel()) * pini_alt #* (1/aug)
