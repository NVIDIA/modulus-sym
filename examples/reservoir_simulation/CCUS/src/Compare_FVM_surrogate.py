...
def ensemble_pytorch(param_perm,param_poro,\
                     nx,ny,nz,Ne,oldfolder,target_min,target_max,minK,maxK,
                     minP,maxP,steppi,device):
...
def Forward_model_ensemble(N,x_true,steppi,
                 target_min,target_max,minK,maxK,minP,maxP,modelP1
                 ,device,
                 num_cores,
                 oldfolder):
...
def compute_metrics_2(y_true1, y_pred_fno1, y_pred_pino1, steppi, name):
    ...
    for i in range(steppi):
        ...
        big_out_ssim[i, 1] = calculate_ssim_3d(y_true1[0, i, :, :, :],
                                                y_pred_fno1[0, i, :, :, :])
        big_out_ssim[i, 2] = calculate_ssim_3d(y_true1[0, i, :, :, :],
                                                y_pred_pino1[0, i, :, :, :])
        
        a1 = calculate_ssim_3d(y_true1[0, i, :, :, :],
                                                y_pred_fno1[0, i, :, :, :])
        a1 = abs(1-a1)
        
        
        a2 = calculate_ssim_3d(y_true1[0, i, :, :, :],
                                                y_pred_pino1[0, i, :, :, :])
        a2 = abs(1-a2)        
    ...
    axs[0, 0].bar(index + bar_width, big_out_r2[:, 2], bar_width, label='PINO', color='r')
    ...
    axs[0, 1].bar(index + bar_width, big_out_l2[:, 2], bar_width, label='PINO', color='r')
    ...
    axs[1, 0].bar(index + bar_width, big_out_ssim[:, 2], bar_width, label='PINO', color='r')
    ...
    axs[1, 1].bar(index + bar_width, big_out_comb[:, 2], bar_width, label='PINO', color='r')
...
def process_step(kk, steppi, dt, pressure, 
                 pressure_true, Swater, Swater_true, 
                 Sgas, Sgas_true, nx, ny, nz, N_injg, 
                 gass,fold,fold1):
    ...
    Plot_Modulus(ax1,nx,ny,nz,look,N_injg,
    'pressure Modulus',gass)            
    ax2 = f_3.add_subplot(3,3,2, projection='3d')
    Plot_Modulus(ax2,nx,ny,nz,lookf,N_injg,
    'pressure Numerical',gass)                        
    ax3 = f_3.add_subplot(3,3,3, projection='3d')
    Plot_Modulus(ax3,nx,ny,nz,diff1,
    N_injg,'pressure diff',gass)         
    ...
    Plot_Modulus(ax2,nx,ny,nz,lookf,N_injg,
    'water Numerical',gass)                        
    ax3 = f_3.add_subplot(3,3,6, projection='3d')
    Plot_Modulus(ax3,nx,ny,nz,diff1,
    N_injg,'water diff',gass)         
    ...
    Plot_Modulus(ax3,nx,ny,nz,diff1,N_injg,
    'gas diff',gass)         
...
folder = path_out
Pr,sw,sg,tt = Geta_all(folder,nx,ny,nz,oldfolder2,check,
                         steppi)
pressure.append(Pr)
Sgas.append(sg)
Swater.append(sw)
...
inn = ensemble_pytorch(perm_use,poro_use,\
                     nx,ny,nz,Ne,oldfolder,target_min,target_max,minK,maxK,
                     minP,maxP,
                     steppi,device)
...
fno_supervised1 = FNOArch([Key("perm", size=1),Key("Phi", size=1), Key("Pini", size=1),\
            Key("Swini", size=1) ],\
dimension=3, decoder_net=decoder1,activation_fn = Activation.RELU) 
...
pressure,Swater,Sgas= Forward_model_ensemble(Ne,inn,steppi,
                 target_min,target_max,minK,maxK,minP,maxP,fno_supervised1
                 ,device,
                 num_cores,
                 oldfolder)   

elapsed_time_secs2 = time.time() - start_time_plots2    
msg = "Reservoir simulation with Nvidia Modulus took: %s secs (Wall clock time)" % timedelta(seconds=round(elapsed_time_secs2))
print(msg)    
print('')
...
plt.ylabel('Time (seconds)', fontweight='bold', fontsize=16)  # Increased font size for ylabel
plt.title('Execution Time Comparison for Modulus vs. Flow', fontweight='bold', fontsize=15)  # Increased font size for title
plt.ylim(0, max(times) * 1.5)  # Increase the upper limit to 150% of the maximum time

# Annotate bars with their values
for bar in bars:
    yval = bar.get_height()
    plt.text(bar.get_x() + bar.get_width()/2, yval + 1, f'{round(yval, 2)}s', ha='center', va='bottom', fontweight='bold', fontsize=15)  # Optional: Adjusted font size for annotations


#plt.show()
# Indicate speedup on the chart
plt.text(0.5, max(times) + 2, f'Speedup: {speedup}x', ha='center', fontsize=14, fontweight='bold', color='blue')  # Adjusted font size for speedup text
...
results = Parallel(n_jobs=num_cores, backend="loky", verbose=10)(delayed(process_step)
                    (kk, steppi, dt, pressure, 
                    pressure_true, Swater, Swater_true, 
                    Sgas, Sgas_true, nx, ny, nz, 
                    N_injg, gass,folderr,oldfolder) for kk in range(steppi))
...
fig4.text(0.5, 0.98, "R2(%) Accuracy - Modulus/Numerical", ha='center', va='center', fontproperties=font, fontsize=11)
fig4.text(0.5, 0.49, "L2(%) Accuracy - Modulus/Numerical", ha='center', va='center', fontproperties=font, fontsize=11)
...
plt.subplot(2,3,3)
plt.plot(Time_vector, Accuracy_gas[:, 0], label='R2', marker='*', markerfacecolor='red', markeredgecolor='red', linewidth=0.5)
...
plt.subplot(2,3,6)
plt.plot(Time_vector, Accuracy_gas[:, 1], label='L2', marker='*', markerfacecolor='red', markeredgecolor='red', linewidth=0.5)
plt.title('gas saturation', fontproperties=font)
plt.xlabel('Time (days)', fontproperties=font)
plt.ylabel('L2(%)', fontproperties=font)
...
decoder_fno = ConvFullyConnectedArch([Key("z", size=32)], \
            [Key("pressure", size=steppi),Key("water_sat", size=steppi),Key("gas_sat", size=steppi)])
    
fno_supervised2 = FNOArch([Key("perm", size=1),Key("Phi", size=1), Key("Pini", size=1),\
            Key("Swini", size=1) ],\
dimension=3, decoder_net=decoder_fno) 
...
pressuref,Swaterf,Sgasf= Forward_model_ensemble(Ne,inn,steppi,
                 target_min,target_max,minK,maxK,minP,maxP,fno_supervised2
                 ,device,
                 num_cores,
                 oldfolder)   
...
results = Parallel(n_jobs=num_cores, backend="loky", verbose=10)(delayed(process_step)
                    (kk, steppi, dt, pressuref, 
                    pressure_true, Swaterf, Swater_true, 
                    Sgasf, Sgas_true, nx, ny, nz, 
                    N_injg, gass,folderr1,oldfolder) for kk in range(steppi))
