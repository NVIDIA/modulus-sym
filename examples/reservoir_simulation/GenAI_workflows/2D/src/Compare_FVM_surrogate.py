...
chuu = None
while True:
    chuu = np.int(input('Select model to use to test the skill of the surrogate -\n\
    1 = Model default\n\
    2 = From realisations\n: '))
    if (chuu>2) or (chuu<1):
        #raise SyntaxError('please select value between 1-2')
...
if chuu ==1: 
    ...
    ini_ensemblef = mat['Z'] 
    index = np.random.choice(ini_ensemblef .shape[1], 1, \
                                     replace=False)  
...
msg = "FVM Reservoir simulation  took: %s secs (Wall clock time)" % timedelta(seconds=round(elapsed_time_secs))
print(msg)
print('') 
...
for kk in range(steppi):   
    ...
    ShowBar(progressBar)
    time.sleep(1)       
    Plot_performance(ouut_p[0,:,:,:],ouut_s[0,:,:,:],\
                data_use1[0,:,:,:],nx,ny,'PINN_model_PyTorch.png',\
                UIR,kk,dt,MAXZ,pini_alt,steppi,wells)            
    Plot_performance(ouut_p[0,:,:,:],ouut_s[0,:,:,:],\
                data_use1[0,:,:,:],nx,ny,'new.png',UIR,kk,dt,MAXZ,\
                pini_alt,steppi,wells)
...
seeTrue = Peaceman_well(inn,data_use1[:,:steppi,:,:],\
                        data_use1[:,steppi:,:,:],MAXZ,0,1e1,LUB,HUB,aay,bby,DX,steppi,\
                pini_alt,SWI,SWR,UW,BW,DZ,rwell,skin,UO,BO,pwf_producer,dt,N_inj,N_pr,nz)
