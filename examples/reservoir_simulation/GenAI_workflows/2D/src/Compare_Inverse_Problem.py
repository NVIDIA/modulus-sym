...
inn2 = {'perm': torch.from_numpy(ini_ensemble1).to(device,torch.float32),\
'Q': torch.from_numpy(ini_ensemble2).to(device,\
dtype=torch.float32),'Qw': torch.from_numpy(ini_ensemble3).to(device,dtype=torch.float32),\
'Phi': torch.from_numpy(ini_ensemble4).to(device,dtype=torch.float32),'Time': \
torch.from_numpy(ini_ensemble5).to(device,dtype=torch.float32),
...
X_data3, data_use3 = inference_single(ini,inip,nx,ny,nz,max_t,DX,DY,DZ,BO,BW,CFL\
                                     ,timmee,MAXZ,factorr,steppi,\
                   LIR,UIR,LUB,HUB,aay,bby,mpor,hpor,dt,IWSw,PB,PATM,CFO,method,\
                    SWI,SWR,UW,UO,typee,step2,pini_alt,input_channel,kka)
...
inn3 = {'perm': torch.from_numpy(ini_ensemble1).to(device,torch.float32),\
'Q': torch.from_numpy(ini_ensemble2).to(device,\
dtype=torch.float32),'Qw': torch.from_numpy(ini_ensemble3).to(device,dtype=torch.float32),\
'Phi': torch.from_numpy(ini_ensemble4).to(device,dtype=torch.float32),'Time': \
torch.from_numpy(ini_ensemble5).to(device,dtype=torch.float32),
...
seeTrue = Peaceman_well2(inn,data_use1[:,:steppi,:,:],\
...
see2 = Peaceman_well2(inn2,data_use2[:,:steppi,:,:],\
                        data_use2[:,steppi:,:,:],MAXZ,0,1e1,LUB,HUB,aay,bby,DX,steppi,\
                pini_alt,SWI,SWR,UW,BW,DZ,rwell,skin,UO,BO,pwf_producer,dt,N_inj,N_pr,nz) 
    
see3 = Peaceman_well2(inn3,data_use3[:,:steppi,:,:],\
                        data_use3[:,steppi:,:,:],MAXZ,0,1e1,LUB,HUB,aay,bby,DX,steppi,\
                pini_alt,SWI,SWR,UW,BW,DZ,rwell,skin,UO,BO,pwf_producer,dt,N_inj,N_pr,nz)    
