    ...
    def forward(self, input_var: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        #Slower but more accurate implementation
           ...
           # water_sat = sat
            ...
            inn_now2 = a1
            dcdx = ddx(inn_now2, dx=dxf, channel=0, dim=0, order=1, padding="replication")
            dcdy = ddx(inn_now2, dx=dxf, channel=0, dim=1, order=1, padding="replication")
            ...
            inn_now2 = a1water
            dadx = ddx(inn_now2, dx=dxf, channel=0, dim=0, order=1, padding="replication")
            dady = ddx(inn_now2, dx=dxf, channel=0, dim=1, order=1, padding="replication")
...
def run(cfg: ModulusConfig) -> None:
    ...
    for kk in range(X_data1.shape[0]):
        ...
        cPress[kk,:,:,:] = np.clip(perm ,1/pini_alt,1.)    
    ...
    for kk in range(X_data2.shape[0]):
        ...
        ini_ensemble8[kk,:,:,:] = np.clip(perm ,1/pini_alt,1.)   
