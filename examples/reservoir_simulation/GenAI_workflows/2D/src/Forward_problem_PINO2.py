...
def CustomValidatorPlotter(timmee,max_t,MAXZ,pini_alt,nx,ny,wells,steppi,tc2,\
            dt,namet,true_water, pred_water,true_pressure, pred_pressure):
    
       
        water_true, water_pred = true_water.detach().cpu().numpy() , pred_water.detach().cpu().numpy() 
        pressure_true, pressure_pred = true_pressure.detach().cpu().numpy() , pred_pressure.detach().cpu().numpy() 
...
def Black_oil(UIR,pini_alt,LUB, HUB,aay, bby,SWI,SWR,UW,BW,UO,BO, MAXZ,
             nx,ny,approach,input_var,device,myloss):
    ...
    #Slower but more accurate implementation
        ...
        for zig in range(sat.shape[0]):
            for count in range (sat.shape[1]):
                ...
                # FDM gradients    
                dudx_fdm = dx(ua, dx=dxf, channel=0, dim=0, order=1, padding="replication")
                dudy_fdm = dx(ua, dx=dyf, channel=0, dim=1, order=1, padding="replication")
                dduddx_fdm = ddx(
                ...
                darcy_pressure = torch.sum(torch.abs(finuse[zig,0,:,:][None,None,:,:].reshape(1,-1)\
                - right.reshape(1,-1))/1)
                # Zero outer boundary
                #darcy_pressure = F.pad(darcy_pressure[:, :, 2:-2, 2:-2], [2, 2, 2, 2], "constant", 0) 
        ...
        #output_var["darcy_pressure"] = torch.mean(p_loss,dim = 0)[None,:,:,:]
                ...
                dduddx = ddx(ua, dx=dxf,channel=0, dim=0, order=1, padding="replication")        
                dduddy = ddx(ua, dx=dyf,channel=0, dim=1, order=1, padding="replication")          
                ...
                toge = flux + finuse
                darcy_saturation = torch.sum(torch.abs(fifth.reshape(1,-1) - toge.reshape(1,-1)))
...
class Labelledset():
    ...
    def __getitem__(self, index):
        ...
        cuda = 0
        device = torch.device(f"cuda:{cuda}" if torch.cuda.is_available() else "cpu")
        return {'perm':x1.to(device,torch.float32),'Q': x2.to(device,torch.float32),\
        'Qw': x3.to(device,torch.float32),'Phi': x4.to(device,torch.float32),\
        'Time': x5.to(device,torch.float32),'Pini': x6.to(device,torch.float32),\
        'Swini': x7.to(device,torch.float32)\
            ,'pressure': x8.to(device,torch.float32),'water_sat': x9.to(device,torch.float32)}
...
class unLabelledset():
    ...
    def __getitem__(self, index):
        ...
        return {'perm':x1.to(device,torch.float32),\
        'Q': x2.to(device,torch.float32),'Qw': x3.to(device,torch.float32),\
        'Phi': x4.to(device,torch.float32),'Time': x5.to(device,torch.float32)\
        ,'Pini': x6.to(device,torch.float32),'Swini': x7.to(device,torch.float32)}
...
def run(cfg: ModulusConfig) -> None:
    ...
    default = None
    while True:
        default = int(input('Select 1 = use default values | 2 = Use user defined values \n'))
    ...
    if use_pretrained == 1:
        ...
        if choice ==1:
            ...
            if (bb==False):            
                ...
                filename = to_absolute_path("../PACKETS/Ganensemble.mat") #Ensemble generated offline
            ...
            if (bb==False):              
                ...
                print('....Downloading Please hold.........')
                download_file_from_google_drive('1Kbe3F7XRzubmDU2bPkfHo2bEmjBNIZ29',\
                            to_absolute_path('../PACKETS/Ganensemble_gauss.mat'))
                print('...Downlaod completed.......')  
                    
                    
                filename = to_absolute_path("../PACKETS/Ganensemble_gauss.mat") #Ensemble generated offline
                mat = sio.loadmat(filename)
                ini_ensemblef = mat['Z']
                scaler1a = MinMaxScaler(feature_range=(aay, bby))
                (scaler1a.fit(ini_ensemblef))
        ...
        if choice ==1:
            ...
            if TII==3:
                ...
                print('....Downloading Please hold.........')
                download_file_from_google_drive('1bib2JAZfBpW4bKz5LdCmhAOtxQixkTzj',\
                to_absolute_path('../Training_Images/iglesias2.out')) 
                print('...Downlaod completed.......')     
                    
                kq=np.genfromtxt(to_absolute_path("../Training_Images/iglesias2.out")\
                ...
                print('....Downloading Please hold.........')    
                download_file_from_google_drive('1TrAVvB-XXCzwHqDdCR4BJnmoe8nPsWIF',\
                to_absolute_path('../Training_Images/TI_2.out')) 
                print('...Downlaod completed.......')  
                
                kq=np.genfromtxt(to_absolute_path("../Training_Images/TI_2.out"),\
                                 skip_header=3, dtype='float')   
                kq=kq.reshape(-1,1)
                clfy = MinMaxScaler(feature_range=(Low_K,High_K))
                (clfy.fit(kq))    
            ...
            ini_ensemblef=see
            sio.savemat(to_absolute_path("../PACKETS/Ganensemble.mat"), {'Z':ini_ensemblef})
        else:
            print('Get initial geology from scratch using Two-point-statistics run')
            ini_ensemblef = initial_ensemble_gaussian(nx,ny,nz,batch_size,\
                                aay,bby)  
        
            scaler1a = MinMaxScaler(feature_range=(aay, bby))
            (scaler1a.fit(ini_ensemblef))
            ini_ensemblef = (scaler1a.transform(ini_ensemblef)) 
            sio.savemat(to_absolute_path("../PACKETS/Ganensemble_gauss.mat"), {'Z':ini_ensembleee})            
      
    X_train, X_test = train_test_split(ini_ensemblef.T, train_size = batch_size+1,\
                                       random_state=42) 
    X_train = X_train.T
    X_test = X_test.T
    ...
    index = np.random.choice(X_train.shape[1], imp, \
                             replace=False)             
    X_data1 = No_Sim(X_train[:,index],nx,ny,nz,max_t,\
    ...
    sio.savemat(to_absolute_path("../PACKETS/simulationsnosim.mat"),{'perm': cPerm,\
            'Q': cQ,'Qw': cQw,\
    'Phi': cPhi,'Time': cTime,'Pini': cPini,'Swini': cSini})  
    ...
    sio.savemat(to_absolute_path("../PACKETS/simulations.mat"),{'perm': cPerm,\
            'Q': cQ,'Qw': cQw,\
    'Phi': cPhi,'Time': cTime,'Pini': cPini,'Swini': cSini\
        ,'pressure': cPress,'water_sat': cSat})  
    ...
    X_data2, data_use2 = inference_single(ini,inip,nx,ny,nz,max_t,DX,DY,DZ,BO,BW,CFL\
    ...
    sio.savemat(to_absolute_path("../PACKETS/single.mat"),{'perm': \
    ini_ensemble1,'Q': ini_ensemble2,'Qw': ini_ensemble3,\
    'Phi': ini_ensemble4,'Time': ini_ensemble5,'Pini': ini_ensemble6,'Swini': ini_ensemble7\
        ,'pressure': ini_ensemble8,'water_sat': ini_ensemble9})          
        
        
    preprocess_FNO_mat(to_absolute_path("../PACKETS/single.mat")) 


    index = np.random.choice(X_train.shape[1], 1, \
                             replace=False) 
    ...
    X_data_test, data_use_test = inference_single(ini,inip,nx,ny,nz,max_t,DX,DY,DZ,BO,BW,CFL\
                                         ,timmee,MAXZ,factorr,steppi,\
                       LIR,UIR,LUB,HUB,aay,bby,mpor,hpor,dt,IWSw,PB,PATM,CFO,method,\
                        SWI,SWR,UW,UO,typee,step2,pini_alt,input_channel,kka) 
    ...
    sio.savemat(to_absolute_path("../PACKETS/singletest.mat"),{'perm': \
    ini_ensemble1,'Q': ini_ensemble2,'Qw': ini_ensemble3,\
    'Phi': ini_ensemble4,'Time': ini_ensemble5,'Pini': ini_ensemble6,'Swini': ini_ensemble7\
        ,'pressure': ini_ensemble8,'water_sat': ini_ensemble9})          
    ...
    fno_pressure = FNOArch([Key("perm", size=1), Key("Q", size=1), Key("Qw", size=1),\
    ...
    fno_saturation = FNOArch([Key("perm", size=1), Key("Q", size=1), Key("Qw", size=1),\
             Key("Phi", size=1), Key("Time", size=1), Key("Pini", size=1),\
    ...
    optimizer_pressure = torch.optim.Adam(fno_pressure.parameters(), lr=learning_rate, weight_decay=cfg.optimizer.weight_decay)
    scheduler_pressure = torch.optim.lr_scheduler.StepLR(optimizer_pressure, step_size=step_size, gamma=gamma)
    
    
    optimizer_sat = torch.optim.Adam(fno_saturation.parameters(), lr=learning_rate, weight_decay=cfg.optimizer.weight_decay)
    ...
    unlabelled_loader = DataLoader(
        dataset,
        batch_size=cfg.batch_size.unlabelled
    )    
    
    test = sio.loadmat(to_absolute_path("../PACKETS/singletest.mat"))
    inn_test_new = {'perm': torch.from_numpy(test['perm']).to(device,torch.float32),\
    'Q': torch.from_numpy(test['Q']).to(device,\
    dtype=torch.float32),'Qw': torch.from_numpy(test['Qw']).to(device,dtype=torch.float32),\
    'Phi': torch.from_numpy(test['Phi']).to(device,dtype=torch.float32),'Time': \
    torch.from_numpy(test['Time']).to(device,dtype=torch.float32),
     'Pini': torch.from_numpy(test['Pini']).to(device,dtype=torch.float32),'Swini': \
     torch.from_numpy(test['Swini']).to(device,dtype=torch.float32)} 
        
    out_test_new = {'pressure': torch.from_numpy(test['pressure']).to(device,torch.float32),\
    'water_sat': torch.from_numpy(test['water_sat']).to(device,\
    dtype=torch.float32)}


    test = sio.loadmat(to_absolute_path("../PACKETS/single.mat"))
    inn_test_new1 = {'perm': torch.from_numpy(test['perm']).to(device,torch.float32),\
    'Q': torch.from_numpy(test['Q']).to(device,\
    dtype=torch.float32),'Qw': torch.from_numpy(test['Qw']).to(device,dtype=torch.float32),\
    'Phi': torch.from_numpy(test['Phi']).to(device,dtype=torch.float32),'Time': \
    torch.from_numpy(test['Time']).to(device,dtype=torch.float32),
    ...
    out_test_new1 = {'pressure': torch.from_numpy(test['pressure']).to(device,torch.float32),\
    'water_sat': torch.from_numpy(test['water_sat']).to(device,\
    dtype=torch.float32)}
    ...
    approach = None
    while True:
        approach = int(input('Select computation of spatial gradients -\n\
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        for inputaa in unlabelled_loader:
            ...
            loss_test1a = MyLossClement((target2_p).reshape(imtest_use,-1), \
                                    (dtrue_test2_p).reshape(imtest_use,-1))            
                
            loss_test2a = MyLossClement((target2_s).reshape(imtest_use,-1),\
            ...
            loss_t1a = MyLossClement((t2_p).reshape(imtest_use,-1), \
                                    (y_p).reshape(imtest_use,-1))            
                
            loss_t2a = MyLossClement((t2_s).reshape(imtest_use,-1),\
                                     (y_s).reshape(imtest_use,-1)) 
            ...
            loss2 = (loss_pde2*cfg.custom.pde_weighting) + (loss_testa + loss_t)*cfg.custom.data_weighting *1e1
        ...
        if (epoch % cfg.training.rec_results_freq) ==0:
            print('-----------------------Plot Results------------------------')
            CustomValidatorPlotter(timmee,max_t,MAXZ,pini_alt,nx,ny,wells,steppi,tc2,\
                        dt,'dynamics_simulations',dtrue_test2_s, target2_s,dtrue_test2_p, target2_p)
            import glob
            frames=[]
            ...
            frames[0].save('pde_simulation_test.gif',format='GIF',\
            ...
            CustomValidatorPlotter(timmee,max_t,MAXZ,pini_alt,nx,ny,wells,steppi,tc2,\
                        dt,'dynamics_simulations',y_s, t2_s ,y_p, t2_p)
            import glob
            frames=[]
            imgs=sorted(glob.glob("*dynamics_simulations*"), key=os.path.getmtime)
            for i in imgs:
            ...
            frames[0].save('pde_simulation_field.gif',format='GIF',\
                            append_images=frames[1:],save_all=True,duration=500,loop=0)
            from glob import glob
            for f3 in glob("*dynamics_simulations*"):
                os.remove(f3)                
        ...
        print(   'TRAINING')        
        if aha<ahnewa:
            print('   FORWARD PROBLEM COMMENT : Loss increased by ' + \
                  str(abs(aha-ahnewa)))
  
        elif aha>ahnewa:
            print('   FORWARD PROBLEM COMMENT : Loss decreased by ' + \
                  str(abs(aha-ahnewa)) )
        ...
        print('    ******************************   ' ) 
        if ah22a<ahnew22a:
            print('   TEST COMMENT FIELD : Loss increased by ' + str(abs(ah22a-ahnew22a)))
        elif ah22a>ahnew22a:
            print('   TEST COMMENT FIELD : Loss decreased by ' + str(abs(ah22a-ahnew22a)) )
        else:
        ...
        print('   Test Field: saturation loss = ' +str(loss_t2a.detach().cpu().numpy())) 
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        #     ,'pressure': x8,'water_sat': x9}            
            inputin = {'perm':inputa['perm'],'Q': inputa['Q'],\
            'Qw': inputa['Qw'],'Phi':inputa['Phi'],\
            'Time': inputa['Time'],'Pini': inputa['Pini'],'Swini':inputa['Swini']}
            ...
            loss_data1 =  MyLossClement((output_p).reshape(cfg.batch_size.grid,-1),\
                            (target['pressure']).reshape(cfg.batch_size.grid,-1))            
                
            loss_data2 = MyLossClement((output_s).reshape(cfg.batch_size.grid,-1),\
            ...
            f_loss2a1, f_water2a1 = Black_oil(UIR,pini_alt,LUB, HUB,aay, bby,SWI,SWR,UW,BW,UO,BO, MAXZ,
                         nx,ny,approach,input_temp,device,myloss)            
            

            f_loss2b1, f_water2b1 = Black_oil(UIR,pini_alt,LUB, HUB,aay, bby,SWI,SWR,UW,BW,UO,BO, MAXZ,
                         nx,ny,approach,input_temp2,device,myloss)  
            ...
            loss_test1 = MyLossClement((target2_p).reshape(1,-1), \
                                    (dtrue_test2_p).reshape(1,-1))            
                
            loss_test2 = MyLossClement((target2_s).reshape(1,-1),\
                                     (dtrue_test2_s).reshape(1,-1)) 
            ...
            loss_t1a = MyLossClement((t2_p).reshape(1,-1), \
                                    (y_p).reshape(1,-1))            
                
            loss_t2a = MyLossClement((t2_s).reshape(1,-1),\
                                     (y_s).reshape(1,-1)) 
            ...
            loss = (loss_data1 * cfg.custom.dataw *1e3)  +\
        ...
        if (epoch % cfg.training.rec_results_freq) ==0:
            ...
            frames[0].save('operator_simulation_test.gif',format='GIF',\
                            append_images=frames[1:],save_all=True,duration=500,loop=0)
            from glob import glob
            for f3 in glob("*dynamics_simulations*"):
                os.remove(f3)
            ...
            frames[0].save('operator_simulation_field.gif',format='GIF',\
                            append_images=frames[1:],save_all=True,duration=500,loop=0)
            from glob import glob
            for f3 in glob("*dynamics_simulations*"):
                os.remove(f3)                
        ...
        print('   pde loss : pressure equation = ' + str(f_loss2.detach().cpu().numpy()))
        print('   pde loss : saturation equation = ' + str(f_water2.detach().cpu().numpy()))     
        print('    ******************************   ' ) 
        if ah2<ahnew2:
        ...
        print('    ******************************   ' ) 
        if ah22<ahnew22:
            print('    TEST COMMENT FIELD : Loss increased by ' + str(abs(ah22-ahnew22)))
        elif ah22>ahnew22:
            print('    TEST COMMENT FIELD : Loss decreased by ' + str(abs(ah22-ahnew22)) )
        else:
            print('    TEST COMMENT FIELD : No change in Loss ')     
    ...
    elapsed_time_secs = time.time() - start_time
    msg = "Model learning Execution took: %s secs (Wall clock time)" % timedelta(seconds=round(elapsed_time_secs))
    print(msg)      
