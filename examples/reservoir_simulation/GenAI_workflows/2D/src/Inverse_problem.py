...
from tensorflow.keras.layers import BatchNormalization,ZeroPadding2D,Dense,\
    Input,LeakyReLU, \
...
def Forward_model_ensemble(modelP,modelS,x_true,rwell,skin,\
                           pwf_producer,mazw,steppi,Ne,DX,pini_alt,UW,BW,UO,BO,\
                              SWI,SWR,DZ,dt,MAXZ):
#### ===================================================================== ####
...
#### ===================================================================== ####
    ...
    for clem in range(Ne):        
        temp = {'perm': x_true['perm'][clem,:,:,:][None,:,:,:],'Q':x_true['Q'][clem,:,:,:][None,:,:,:], \
        'Qw':x_true['Qw'][clem,:,:,:][None,:,:,:],\
        'Phi':x_true['Phi'][clem,:,:,:][None,:,:,:],'Time':x_true['Time'][clem,:,:,:][None,:,:,:],\
        'Pini':x_true['Pini'][clem,:,:,:][None,:,:,:],'Swini':x_true['Swini'][clem,:,:,:][None,:,:,:]}
    ...
    simulated = []
    for amm in range(Ne):
        Injector_location = np.where(inn["Qw"][amm,0,:,:].detach().cpu().numpy().ravel()>0)[0]
        producer_location = np.where(inn["Q"][amm,0,:,:].detach().cpu().numpy().ravel()<0)[0]
            
        PERM = rescale_linear_pytorch_numpy(np.reshape(inn["perm"][amm,0,:,:].\
                                                detach().cpu().numpy(),(-1,),\
                                    'F'),LUB,HUB, aay, bby) 
        ...
        if a[2]==2:
           plt.scatter(XX.T[xloc-1,yloc-1]+0.5,YY.T[xloc-1,yloc-1]+0.5
            ...
            plt.scatter(XX.T[xloc-1,yloc-1]+0.5,YY.T[xloc-1,yloc-1]+\
                        0.5,s=100,marker='v', color = 'white') 
...
def ensemble_pytorch(param_perm,param_poro,LUB, HUB,mpor, hpor,inj_rate,dt,IWSw,\
                     nx,ny,nz,Ne,input_channel):
    """
    Input:
    ...
    for kk in range(Ne):        
        ...
        ini_ensemble7[kk,0,:,:] = IWSw * np.ones((nx,ny)) #INitial water saturation
    ...
    'Q': torch.from_numpy(ini_ensemble2).to(device,\
    dtype=torch.float32),'Qw': torch.from_numpy(ini_ensemble3).to(device,dtype=torch.float32),\
    'Phi': torch.from_numpy(ini_ensemble4).to(device,dtype=torch.float32),'Time': \
    torch.from_numpy(ini_ensemble5).to(device,dtype=torch.float32),
     'Pini': torch.from_numpy(ini_ensemble6).to(device,dtype=torch.float32),'Swini': \
     torch.from_numpy(ini_ensemble7).to(device,dtype=torch.float32)}    
    return inn
...
def Plot_petrophysical(permmean,poroo,nx,ny,nz,Low_K,High_K):
    ...
    plt.tight_layout(rect = [0,0,1,0.95])
    plt.suptitle('Petrophysical Reconstruction',\
                 fontsize = 25)
...
def Plot_RSM_percentile(pertoutt,True_mat,Namesz):
    ...
    plt.plot(timezz,P90[:,5], color = 'green', lw = '2', \
         label ='P90 Model') 
    plt.plot(timezz,arekibest[:,5], color = 'k', lw = '2', \
         label ='aREKI cum best Model')  
    ...
    plt.plot(timezz,P90[:,6], color = 'green', lw = '2', \
         label ='P90 Model')  
    plt.plot(timezz,arekibest[:,6], color = 'k', lw = '2', \
         label ='aREKI cum best Model')  
    ...
    plt.plot(timezz,arekibest[:,7], color = 'k', lw = '2', \
         label ='aREKI cum best Model')  
    ...
    plt.plot(timezz,arekibest[:,8], color = 'k', lw = '2', \
         label ='aREKI cum best Model')  
    ...
    plt.plot(timezz,arekibest[:,9], color = 'k', lw = '2', \
         label ='aREKI cum best Model')  
    ...
    plt.plot(timezz,arekibest[:,10], color = 'k', lw = '2', \
         label ='aREKI cum best Model')  
    ...
    plt.plot(timezz,P90[:,11], color = 'green', lw = '2', \
         label ='P90 Model')  
    plt.plot(timezz,arekibest[:,11], color = 'k', lw = '2', \
         label ='aREKI cum best Model')  
    ...
    plt.plot(timezz,P90[:,12], color = 'green', lw = '2', \
         label ='P90 Model') 
    plt.plot(timezz,arekibest[:,12], color = 'k', lw = '2', \
         label ='aREKI cum best Model')  
    ...
    plt.plot(timezz,arekibest[:,13], color = 'k', lw = '2', \
         label ='aREKI cum best Model')  
    ...
    plt.plot(timezz,arekibest[:,14], color = 'k', lw = '2', \
         label ='aREKI cum best Model')  
    ...
    plt.plot(timezz,arekibest[:,15], color = 'k', lw = '2', \
         label ='aREKI cum best Model')  
    ...
    plt.plot(timezz,P90[:,16], color = 'green', lw = '2', \
         label ='P90 Model') 
    plt.plot(timezz,arekibest[:,16], color = 'k', lw = '2', \
         label ='aREKI cum best Model')  
...
def Plot_RSM(predMatrix,True_mat,Namesz,Ne):
    ...
    plt.subplot(3,4,1)
    plt.plot(timezz,WOPRA[:,:Ne],color = 'grey', lw = '2',\
             label = 'Realisations')
    plt.xlabel('Time (days)',fontsize = 13)
    plt.ylabel('$Q_{oil}(bbl/day)$',fontsize = 13)
    plt.title('P1',fontsize = 13)
    ...
    plt.subplot(3,4,2)
    plt.plot(timezz,WOPRB[:,:Ne],color = 'grey', lw = '2',\
             label = 'Realisations')
    plt.xlabel('Time (days)',fontsize = 13)
    plt.ylabel('$Q_{oil}(bbl/day)$',fontsize = 13)
    plt.title('P2',fontsize = 13)
    ...
    plt.subplot(3,4,3)
    plt.plot(timezz,WOPRC[:,:Ne],color = 'grey', lw = '2',\
             label = 'Realisations')
    plt.xlabel('Time (days)',fontsize = 13)
    plt.ylabel('$Q_{oil}(bbl/day)$',fontsize = 13)
    plt.title('P3',fontsize = 13)
    ...
    plt.subplot(3,4,4)
    plt.plot(timezz,WOPRD[:,:Ne],color = 'grey', lw = '2',\
             label = 'Realisations')
    plt.xlabel('Time (days)',fontsize = 13)
    plt.ylabel('$Q_{oil}(bbl/day)$',fontsize = 13)
    plt.title('P4',fontsize = 13)
    ...
    plt.subplot(3,4,5)
    plt.plot(timezz,WWPRA[:,:Ne],color = 'grey', lw = '2',\
             label = 'Realisations')
    plt.xlabel('Time (days)',fontsize = 13)
    plt.ylabel('$Q_{water}(bbl/day)$',fontsize = 13)
    plt.title('P1',fontsize = 13)
    ...
    plt.subplot(3,4,6)
    plt.plot(timezz,WWPRB[:,:Ne],color = 'grey', lw = '2',\
             label = 'Realisations')
    plt.xlabel('Time (days)',fontsize = 13)
    plt.ylabel('$Q_{water}(bbl/day)$',fontsize = 13)
    plt.title('P2',fontsize = 13)
    ...
    plt.subplot(3,4,7)
    plt.plot(timezz,WWPRC[:,:Ne],color = 'grey', lw = '2',\
             label = 'Realisations')
    plt.xlabel('Time (days)',fontsize = 13)
    plt.ylabel('$Q_{water}(bbl/day)$',fontsize = 13)
    plt.title('P3',fontsize = 13)
    ...
    plt.subplot(3,4,8)
    plt.plot(timezz,WWPRD[:,:Ne],color = 'grey', lw = '2',\
             label = 'Realisations')
    plt.xlabel('Time (days)',fontsize = 13)
    plt.ylabel('$Q_{water}(bbl/day)$',fontsize = 13)
    plt.title('P4',fontsize = 13)
    ...
    plt.subplot(3,4,9)
    plt.plot(timezz,WWCTA[:,:Ne],color = 'grey', lw = '2',\
             label = 'Realisations')
    plt.xlabel('Time (days)',fontsize = 13)
    plt.ylabel('$WWCT{%}$',fontsize = 13)
    plt.title('P1',fontsize = 13)
    ...
    plt.subplot(3,4,10)
    plt.plot(timezz,WWCTB[:,:Ne],color = 'grey', lw = '2',\
             label = 'Realisations')
    plt.xlabel('Time (days)',fontsize = 13)
    plt.ylabel('$WWCT{%}$',fontsize = 13)
    plt.title('P1',fontsize = 13)
    ...
    plt.subplot(3,4,11)
    plt.plot(timezz,WWCTC[:,:Ne],color = 'grey', lw = '2',\
             label = 'Realisations')
    plt.xlabel('Time (days)',fontsize = 13)
    plt.ylabel('$WWCT{%}$',fontsize = 13)
    plt.title('P3',fontsize = 13)
    ...
    plt.subplot(3,4,12)
    plt.plot(timezz,WWCTD[:,:Ne],color = 'grey', lw = '2',\
             label = 'Realisations')
    plt.xlabel('Time (days)',fontsize = 13)
    plt.ylabel('$WWCT{%}$',fontsize = 13)
    plt.title('P4',fontsize = 13)
...
def DenosingAutoencoder(nx,ny,nz,High_K,Low_K):
    ...
    mc = ModelCheckpoint("../PACKETS/denosingautoencoder.h5", monitor='val_loss', mode='min', \
...
def DenosingAutoencoderp(nx,ny,nz,N_ens,High_P,High_K,Low_K):
    ...
    es = EarlyStopping(monitor='val_loss', mode='min', verbose=1, patience=100)
    mc = ModelCheckpoint("../PACKETS/denosingautoencoderp.h5", monitor='val_loss', mode='min', \
                         verbose=1, save_best_only=True)    
...
def Autoencoder2(nx,ny,nz,High_K,Low_K):
    ...
    es = EarlyStopping(monitor='val_loss', mode='min', verbose=1, patience=100)
    mc = ModelCheckpoint("../PACKETS/autoencoder.h5", monitor='val_loss', mode='min', \
                         verbose=1, save_best_only=True)    
...
def Autoencoder2p(nx,ny,nz,High_P,High_K,Low_K):
    ...
    es = EarlyStopping(monitor='val_loss', mode='min', verbose=1, patience=100)
    mc = ModelCheckpoint("../PACKETS/autoencoderp.h5", monitor='val_loss', mode='min', \
                         verbose=1, save_best_only=True)    
...
def Select_TI(oldfolder,ressimmaster,N_small,nx,ny,nz,True_data,Low_K,High_K):
        ...
        ensemblepy = ensemble_pytorch(ini_ensemble1,ensemblep,LUB, HUB,mpor,\
                                      hpor,inj_rate,dt,IWSw,\
                             nx,ny,nz,ini_ensemble1.shape[1],input_channel) 
        
        mazw = 0 # Dont smooth the presure field
        simDatafinal,predMatrix,_,_,_ = \
            Forward_model_ensemble(modelP,modelS,ensemblepy,rwell,skin,\
                               pwf_producer,mazw,steppi,ini_ensemble1.shape[1]\
                                   ,DX,pini_alt,UW,BW,UO,BO,\
                                SWI,SWR,DZ,dt,MAXZ)
       ...
       # kti2=
        ...
        ensemblepy = ensemble_pytorch(ini_ensemble2,ensemblep,LUB, HUB,mpor,\
                                      hpor,inj_rate,dt,IWSw,\
                             nx,ny,nz,ini_ensemble2.shape[1],input_channel) 
        
        mazw = 0 # Dont smooth the presure field
        simDatafinal,predMatrix,_,_,_ = \
            Forward_model_ensemble(modelP,modelS,ensemblepy,rwell,skin,\
                               pwf_producer,mazw,steppi,ini_ensemble2.shape[1]\
                                   ,DX,pini_alt,UW,BW,UO,BO,\
                                SWI,SWR,DZ,dt,MAXZ)          
        ...
        ensemblepy = ensemble_pytorch(ini_ensemble3,ensemblep,LUB, HUB,mpor,\
                                      hpor,inj_rate,dt,IWSw,\
                             nx,ny,nz,ini_ensemble3.shape[1],input_channel) 
        
        mazw = 0 # Dont smooth the presure field
        simDatafinal,predMatrix,_,_,_ = \
            Forward_model_ensemble(modelP,modelS,ensemblepy,rwell,skin,\
                               pwf_producer,mazw,steppi,ini_ensemble3.shape[1],DX,pini_alt,UW,BW,UO,BO,\
                                SWI,SWR,DZ,dt,MAXZ)
        aa,bb,cc=funcGetDataMismatch(simDatafinal,True_data)
        cc=cc.reshape(-1,1)
        ...
        ensemblepy = ensemble_pytorch(ini_ensemble4,ensemblep,LUB, HUB,mpor,\
                                      hpor,inj_rate,dt,IWSw,\
                             nx,ny,nz,ini_ensemble4.shape[1],input_channel) 
        
        mazw = 0 # Dont smooth the presure field
        simDatafinal,predMatrix,_,_,_ = \
            Forward_model_ensemble(modelP,modelS,ensemblepy,rwell,skin,\
                               pwf_producer,mazw,steppi,ini_ensemble4.shape[1],DX,pini_alt,UW,BW,UO,BO,\
                                SWI,SWR,DZ,dt,MAXZ)
        aa,bb,cc=funcGetDataMismatch(simDatafinal,True_data)
        cc=cc.reshape(-1,1)
        ...
        ensemblepy = ensemble_pytorch(ini_ensemble5,ensemblep,LUB, HUB,mpor,\
                                      hpor,inj_rate,dt,IWSw,\
                             nx,ny,nz,ini_ensemble5.shape[1],input_channel) 
        
        mazw = 0 # Dont smooth the presure field
        simDatafinal,predMatrix,_,_,_ = \
            Forward_model_ensemble(modelP,modelS,ensemblepy,rwell,skin,\
                               pwf_producer,mazw,steppi,ini_ensemble5.shape[1],DX,pini_alt,UW,BW,UO,BO,\
                                SWI,SWR,DZ,dt,MAXZ)
        aa,bb,cc=funcGetDataMismatch(simDatafinal,True_data)
        cc=cc.reshape(-1,1)
        ...
        ensemblepy = ensemble_pytorch(ini_ensembleG,EnsemblepG,LUB, HUB,mpor,\
                                      hpor,inj_rate,dt,IWSw,\
                             nx,ny,nz,ini_ensembleG.shape[1],input_channel) 
        
        mazw = 0 # Dont smooth the presure field
        simDatafinal,predMatrix,_,_,_ = \
            Forward_model_ensemble(modelP,modelS,ensemblepy,rwell,skin,\
                               pwf_producer,mazw,steppi,ini_ensembleG.shape[1],DX,pini_alt,UW,BW,UO,BO,\
                                SWI,SWR,DZ,dt,MAXZ)
        aa,bb,cc=funcGetDataMismatch(simDatafinal,True_data)
        cc=cc.reshape(-1,1)
        cle=np.argmin(cc)
...
def Learn_Overcomplete_Dictionaryp(N_ens):
    ...
    a=dict_learning(ini_ensembleyy.T,Ne,0,max_iter=100,method='lars',\
                    n_jobs=-1,return_n_iter=True,\
                        method_max_iter=10000,verbose=True)
    second=a[1] 
...
def Sparse_coding(Dictionary,inpuut,N_ens):
    ...
    codde=sparse_encode(inpuut.T, Dictionary.T, n_jobs=-1,n_nonzero_coefs=N_ens,\
                verbose=1)
    return codde.T
...
def Plot_Histogram_now(N,E11,E12,mean_cost,best_cost):
    ...
    plt.subplot(2,2,3)
    plt.plot(timezz,mean_cost, color = 'green', lw = '2',\
             label ='mean_model_cost')
    plt.plot(timezz,best_cost, color = 'blue', lw = '2', \
             label ='best_model_cost')        
...
def REKI_ASSIMILATION_LEVELSET(sgsim,ensemblep,simDatafinal,alpha,\
    True_data,Ne,pertubations,Experts,Yet,nx,ny,nz,High_K,Low_K,\
        High_P,Low_P,CDd):
    #sgsim=ensemble
    ...
    Xspitbk=Parallel(n_jobs=8,backend='loky', verbose=0)(delayed(
    Narrow_Band)(sdiuse[:,kk],nx,ny,nz)for kk in range(Ne) )
    ...
    Narrow_Band)(sdiusep[:,kk],nx,ny,nz)for kk in range(Ne) )
    ...
    overall=cp.vstack([cp.asarray(sgsim),cp.asarray(sdiuse),\
    cp.asarray(ensemblep),cp.asarray(sdiusep)]) 
        
    overallB=cp.vstack([B_pixelK,cp.asarray(B),\
    B_pixelporo,cp.asarray(Bp)])            
    ...
    Cdd=GDT.T@GDT
    Cyd=Cydd@GDT
    Usig,Sig,Vsig = cp.linalg.svd((Cdd + (cp.asarray(alpha)*cp.eye(CDd.shape[1]))), \
                                  full_matrices = False)
    ...
    update_term=((Cyd@(X))@(inv_CDd)@((cp.tile(cp.asarray(True_data), Ne)\
    ...
    Xspit2k=Parallel(n_jobs=num_cores,backend='loky', verbose=0)(delayed(
    rectifyy)(updated_ensemble[:,k],updated_ensemble_LS[:,k],meximkp[:,k],\
    ...
    Xspit2kp=Parallel(n_jobs=num_cores,backend='loky', verbose=0)(delayed(
    rectifyy)(updated_ensemblep[:,k],updated_ensemblep_LS[:,k],meximkpp[:,k],\
                    Experts,nx,ny,nz,models_bigp[k])for k in range(Ne) )
    updated_yesP = np.reshape(np.hstack(Xspit2kp),(-1,Ne),'F')#           
    #updated_yesP = np.hstack(Xspit2kp)
    #Honour well pixel values
    ensemblee,ensembleep=honour2(updated_yesP,\
                                 updated_yesK,nx,ny,nz,Ne,\
...
def REKI_ASSIMILATION_NORMAL_SCORE(sgsim,ensemblepi,simDatafinal,alpha,\
    True_data,Ne,pertubations,Yet,nx,ny,nz,High_K,Low_K,\
        High_P,Low_P,CDd):
      
    
    sizeclem=cp.asarray(nx*ny*nz) 
    
    quantileperm = QuantileTransformer(n_quantiles=nx*ny*nz,\
                                       output_distribution='normal',random_state=0)
    quantileporo = QuantileTransformer(n_quantiles=nx*ny*nz,\
                                       output_distribution='normal',random_state=0)
    ...
    overall=cp.vstack([cp.asarray(sgsimm),\
    cp.asarray(ensemblepp)]) 
    ...
    (V,X,U) = pinvmatt((Cdd + (cp.asarray(alpha)*cp.eye(CDd.shape[1]))),tol)
    innovation=((cp.tile(cp.asarray(True_data), Ne)\
                                + (cp.sqrt(cp.asarray(alpha))*\
                                cp.asarray(pertubations)))\
                                  -Sim1 )
    # if Yet==0:
    ...
    #Honour well pixel values
    ensemblee,ensembleep=honour2(updated_yesP,updated_yesK,nx,ny,nz,Ne,\
                    High_K,Low_K,High_P,Low_P)          
...
class WGAN():
    ...
    def train(self,dataa, epochs, batch_size=128, sample_interval=50):
        ...
        X_train=dataa
        # Rescale -1 to 1
        X_train =(X_train /(High_K/2))-1# 127.5 - 1.# (X_train.astype(np.float32) - 127.5) / 127.5
...
class WGANP():
    ...
    def train(self,dataa, epochs, batch_size=128, sample_interval=50):
        ...
        X_train=dataa
        # Rescale -1 to 1
        X_train =(X_train /(High_P/2))-1# 127.5 - 1.# (X_train.astype(np.float32) - 127.5) / 127.5
...
def Make_Bimodal(matrix,nclusters,High_K,Low_K):
    ...
    for kk in range(matrix.shape[1]):
        kmeans =MiniBatchKMeans(n_clusters=nclusters,max_iter=2000).\
            fit(matrix[:,kk].reshape(-1,1))
        dd=kmeans.labels_
        dd=dd.T
        dd=np.reshape(dd,(-1,1))
...
def Regress(X_train,y_traind,namezz):  
    ...
    if gff<1:
        gff=1
    modelDNN.fit(a0, b0,validation_split=0.1, batch_size =gff, verbose=0, \
...
def classify(X_train,y_traind,namezz):
    ...
    mc = ModelCheckpoint(namezz, monitor='val_accuracy', mode='max', \
                         verbose=0, save_best_only=True)
    a0=X_train
    a0=np.reshape(a0,(-1,X_train.shape[1]),'F')
...
def Variational_Autoencoder(latent_dim,nx,ny,nz,High_K,Low_K,mem):
    ...
    x = tensorflow.keras.layers.Conv2D(32, 3, activation="gelu", strides=1, padding="same")(encoder_inputs)
    x = tensorflow.keras.layers.Conv2D(64, 3, activation="gelu", strides=3, padding="same")(x)
    ...
    x = tensorflow.keras.layers.Conv2DTranspose(64, 3, activation="gelu", strides=3, padding="same")(x)
    x = tensorflow.keras.layers.Conv2DTranspose(32, 3, activation="gelu", strides=1, padding="same")(x)
    decoder_outputs = tensorflow.keras.layers.Conv2DTranspose(nz, 3, activation="linear", padding="same")(x)
...
if DEFAULT ==1:
    ...
    surrogate = None
    while True:
        surrogate=int(input('Select surrogate method type:\n1=FNO [Modulus Implementation]\n\
...
pini = 1. # initial reservoir pressure
pini_alt = 1e3
sat_up = pini_alt*10
...
max_rwell = 500. #  maximum well radius (ft)
min_rwell = 40. #  minimum well radius (ft)
max_skin = 0.02 # maximum skin factor
min_skin = 0 # minimum skin factor
max_pwf_producer = 300. # maximum pwf in psi controller for producer
min_pwf_producer = 20. # maximum pwf in psi controller for producer
...
timmee =  100.#float(input ('Enter the time step interval duration for simulation (days): '))
max_t = 3000.#float(input ('Enter the maximum time in days for simulation(days): '))
t_plot = int(timmee)#int(input ('Enter the plot frequency for simulation (days): ')) # plot every ...
MAXZ = 6000
steppi = int(max_t/timmee)    
ntrain = 5000#int((int(np.ceil((sizq - batch_size)/batch_size)) - steppi )* batch_size) \
    #+ (batch_size *2)
...
if DEFAULT==1:
    ...
    weighting = None
    while True:
        weighting = int(input('Enter the weighting scheme for model realisations:\n\
1 = weigthing\n\
2 = partial-weighting\n\
3 = Non-weighting\n\
        '))
...
if DEFAULT == 1:
    ...
    method = None
    while True:
        method = cp.int(input('Select inverse problem solution method for pressure & saturation \
        solve -\n\
    ...
    if method == 6:
    	typee = cp.int(input('Select JSON config -\n\
    1 = Standard\n\
    2 = Exotic\n\
    3 = DILU\n\
    4 = Aggregation DS\n\
...
if DEFAULT==1:
    ...
    use_pretrained = None
    while True:
        use_pretrained = int(input('Use pretrained models:\n\
        1 = Yes\n\
        2 = No\n\
        '))
...
if surrogate == 4:
    ...
    modelP = FNOArch([Key("perm", size=1),Key("Q", size=1),Key("Qw", size=1),\
             Key("Phi", size=1),Key("Time", size=1),Key("Pini", size=1),\
                Key("Swini", size=1) ], dimension=2, decoder_net=decoder1)  

    decoder2 = ConvFullyConnectedArch([Key("z", size=32)], \
            [Key("water_sat", size=steppi),Key("Txw", size=steppi),Key("Tyw", size=steppi)])
    modelS = FNOArch([Key("perm", size=1),Key("Q", size=1),Key("Qw", size=1),\
             Key("Phi", size=1),Key("Time", size=1),Key("Pini", size=1),\
                Key("Swini", size=1) ], dimension=2, decoder_net=decoder2)    
...
if (surrogate == 5) or (surrogate == 6) :       
    ...
    modelP = AFNOArch([Key("perm", size=1),Key("Q", size=1),Key("Qw", size=1),\
    Key("Phi", size=1), Key("Time", size=1), Key("Pini", size=1),\
    Key("Swini", size=1)], [Key("pressure", size=steppi)], (nx, ny),patch_size = 3) 
        
    # Define AFNO model for forward model (saturation)
    modelS = AFNOArch([Key("perm", size=1),Key("Q", size=1),Key("Qw", size=1),\
    Key("Phi", size=1), Key("Time", size=1), Key("Pini", size=1),\
    Key("Swini", size=1)], [Key("water_sat", size=steppi)], (nx, ny),patch_size = 3)  
...
if surrogate ==1:
    ...
    bb = os.path.isfile("outputs/Forward_problem_FNO/ResSim/fno_forward_model_pressure.0.pth")
    if (bb==False): 
        print('....Downloading Please hold.........')  
        download_file_from_google_drive('1cldZ75k-kIJQU51F1w17yRYFAAjoYOXU',\
    ...
    bba = os.path.isfile("outputs/Forward_problem_FNO/ResSim/fno_forward_model_saturation.0.pth")
    if (bba==False):  
        print('....Downloading Please hold.........')               
        download_file_from_google_drive('1IWTnWceqbCD3XdQmHOsw6Et9jS8hozML',\
    ...
    if not os.path.exists(('outputs/Forward_problem_PINO/ResSim/')):
        ...
        pass      
    bb = os.path.isfile("outputs/Forward_problem_PINO/ResSim/pino_forward_model_pressure.0.pth")
    if (bb==False):      
        print('....Downloading Please hold.........')      
        download_file_from_google_drive('1Df3NHyAMW4fdAVwdSyQEvuD8Z9gpsNwt',\
    ...
    bba = os.path.isfile("outputs/Forward_problem_PINO/ResSim/pino_forward_model_saturation.0.pth")
    if (bba==False):      
        print('....Downloading Please hold.........')     
        download_file_from_google_drive('1QAYQJy9_2FiBrxL8TYtTvTJNmsTaUSh4',\
    ...
    if (bb==False):      
        print('....Downloading Please hold.........') 
        download_file_from_google_drive('1sv_tbB91EWcJOBWdhhnTKspoAtR564o4',\
    ...
    if (bba==False):      
        print('....Downloading Please hold.........')     
        download_file_from_google_drive('13BGxwvN2IV0eo0vbUxb16BhJ2MtJ-s0W',\
    ...
    print ('-----------------Surrogate Model learned with Original PINO Version 2----------------')       
    ...
    if (bb==False):      
        print('....Downloading Please hold.........')   
        download_file_from_google_drive('1jihu5qi0TWh22RuIxNEvUxvixUrnxzxB',\
    ...
    if (bba==False):      
        print('....Downloading Please hold.........')     
        download_file_from_google_drive('1FibuOGfBZHOrZgiPf6lggcCpKlaMaq7A',\
    ...
    bb = os.path.isfile("outputs/Forward_problem_AFNOD/ResSim/afno_forward_model_pressure.0.pth")
    if (bb==False): 
        print('....Downloading Please hold.........')  
        download_file_from_google_drive('1v1s5WtJ4HVYb1_Qm8GRLxlEbpOqzxMZU',\
    ...
    bba = os.path.isfile("outputs/Forward_problem_AFNOD/ResSim/afno_forward_model_saturation.0.pth")
    if (bba==False):  
        print('....Downloading Please hold.........')               
        download_file_from_google_drive('1UdAH2WyXdY87VncoDUjiM-SzmnSSpelY',\
    ...
    bb = os.path.isfile("outputs/Forward_problem_AFNOP/ResSim/afnop_forward_model_pressure.0.pth")
    if (bb==False): 
        print('....Downloading Please hold.........')  
        download_file_from_google_drive('14Iq6ERuHfrd6u5HMiUVNU3zzaJRusnmJ',\
    ...
    bba = os.path.isfile("outputs/Forward_problem_AFNOP/ResSim/afnop_forward_model_saturation.0.pth")
    if (bba==False):  
        print('....Downloading Please hold.........')               
        download_file_from_google_drive('1qazyOOIfoXRHyJ99kfW3vICFEtPtR9sV',\
...
wells = np.array([1,24,1,3,3,1,31,1,1,31,31,1,7,9,2,
                  14,12,2,28,19,2,14,27,2])
...
if (bba==False):
    print('....Downloading Please hold.........')
    download_file_from_google_drive('1_9VRt8tEOF6IV7GvUnD7CFVM40DMHkxn',\
    '../PACKETS/iglesias2.out') 
    print('...Downlaod completed.......')
...
if DEFAULT == 1:
    ...
    BASSE = None
    while True:
        BASSE=int(input('Generate the covariance noise matrix using\n\
...
if DEFAULT ==1:
    Technique_REKI = 3
    print('Default method is the Weighted  Adaptive Ensemble Kalman Inversion + Convolution Autoencoder ')
else:
    
    Technique_REKI = None
    while True:
        Technique_REKI=int(input('Enter optimisation with the Adaptive Ensemble Kalman Inversion: \n\
...
if DEFAULT ==1:
    ...
    if (bb==False):      
        print('....Downloading Please hold.........')    
        download_file_from_google_drive('1I-27_S53ORRFB_hIN_41r3Ntc6PpOE40',\
        '../PACKETS/Training4.mat')
        print('...Downlaod completed.......')  
    ...
    Ne = None
    while True:
        Ne = int(input('Number of realisations used for history matching (100-2000) : ')) 
    ...
    if Ne < ini_ensemble.shape[1]:
        index = np.random.choice(ini_ensemble.shape[1], Ne, \
                                 replace=False) 
    ...
    Ne = None
    while True:
        Ne = int(input('Number of realisations used for history matching (100-9000) : ')) 
    ...
    if Geostats==1:         
        if afresh==1:
            ...
            ensemblepy = ensemble_pytorch(ensemble,ensemblep,LUB, HUB,mpor,\
                                          hpor,inj_rate,dt,IWSw,\
                                 nx,ny,nz,ensemble.shape[1],input_channel) 
            
            mazw = 0 # Dont smooth the presure field
            simDataa,_,_,_,_ = \
                Forward_model_ensemble(modelP,modelS,ensemblepy,rwell,skin,\
                                   pwf_producer,mazw,steppi,ensemble.shape[1],DX,\
                                       pini_alt,UW,BW,UO,BO,\
                                    SWI,SWR,DZ,dt,MAXZ)    
            True_data = np.reshape(True_data,(-1,1),'F')
            ...
            if Ne <=2000:
                ...
                if (bb==False):            
                    ...
                    print('....Downloading Please hold.........')
                    download_file_from_google_drive('1w81M5M2S0PD9CF2761dxmiKQ5c0OFPaH',\
                ...
                mat = sio.loadmat(filename)
                ini_ensemblef=mat['Z']
                index = np.random.choice(ini_ensemblef.shape[1], Ne+100, \
                replace=False) 
        ...
        if afresh==1:
            ini_ensemblef = initial_ensemble_gaussian(nx,ny,nz,(N_ens + 10),\
                                Low_K,High_K) 
            ...
            if (bb==False):            
                ...
                print('....Downloading Please hold.........')
                download_file_from_google_drive('1Kbe3F7XRzubmDU2bPkfHo2bEmjBNIZ29', filename)
                print('...Downlaod completed.......')
                print('')
            else:
                pass              
...
if Technique_REKI==1:
    print('')
    print('Adaptive Regularised Ensemble Kalman Inversion with Denoising Autoencoder \
post-processing\n')
    ...
    if (Geostats==1):
        ...
        if (bb==False) :
            if use_pretrained == 1: 
                print('....Downloading Please hold.........')
                download_file_from_google_drive('17Ekju_MvRs_oOhDvpfirp8-rFy6x1D2u',\
        ...
        if (bb2==False) : 
            ...
            if use_pretrained == 1: 
                print('....Downloading Please hold.........')
                download_file_from_google_drive('1YBk_xwT175rOg3wbaTRZ6ZaCASPsvqoL',\
    ...
    pertubations=cp.random.multivariate_normal(cp.ravel(cp.zeros\
                ((Nop,1))), Cini,Ne,method='eigh',dtype=cp.float32)
    ...
    while (snn<1):        
        ...
        if Yet==0:
            ...
            pass
        yyy=np.mean(0.5*((Dd-simDatafinal).T@(inv(CDd))\
        ...
        if ii==0:
            ...
            tinuke=((np.sum((((simmean) - True_dataa) ** 2)) )  **(0.5))\
                /True_dataa.shape[0]
            print('Initial RMSE of the ensemble mean = : ' \
                  + str(tinuke) + '... .') 
            aa,bb,cc=funcGetDataMismatch(simDatafinal,True_dataa)
            clem=np.argmin(cc)
            simmbest=simDatafinal[:,clem].reshape(-1,1)
            tinukebest=((np.sum((((simmbest) - True_dataa) ** 2)) )  **(0.5))\
                /True_dataa.shape[0]
            print('Initial RMSE of the ensemble best = : ' \
                + str(tinukebest) + '... .')
            cc_ini=cc
            ...
            if tinuke<tinumeanprior:
                print('ensemble mean cost decreased by = : ' \
                      + str(abs(tinuke-tinumeanprior)) + '... .')
                    

                   
            if tinuke>tinumeanprior:
                print('ensemble mean cost increased by = : ' \
                      + str(abs(tinuke-tinumeanprior)) + '... .')
            ...
            if tinukebest>tinubestprior:
                print('ensemble best cost increased by = : ' \
                      + str(abs(tinukebest-tinubestprior)) + '... .')  
               
                    
            if tinukebest<tinubestprior:
                print('ensemble best cost decreased by = : ' \
                      + str(abs(tinukebest-tinubestprior)) + '... .')
    ...
    ensemble,ensemblep=honour2(use_p,\
                             use_k,nx,ny,nz,N_ens,\
                                 High_K1,Low_K1,High_P,Low_P)   

    all_ensemble,all_ensemblep=honour2(all_ensemblep,\
                             all_ensemble,nx,ny,nz,N_ens,\
                                 High_K1,Low_K1,High_P,Low_P)            
    ...
    mazw = 0 # Dont smooth the presure field
    simDatafinal,predMatrix,pressure_ensemble,water_ensemble,oil_ensemble = \
        Forward_model_ensemble(modelP,modelS,ensemblepy,rwell,skin,\
                           pwf_producer,mazw,steppi,ensemble.shape[1],DX,pini_alt,UW,BW,UO,BO,\
                            SWI,SWR,DZ,dt,MAXZ) 
    ensemblepya = ensemble_pytorch(all_ensemble,all_ensemblep,LUB, HUB,mpor,\
                              hpor,inj_rate,dt,IWSw,\
                     nx,ny,nz,ensemble.shape[1],input_channel) 

    mazw = 0 # Dont smooth the presure field
    simDatafinala,predMatrixa,pressure_ensemblea,water_ensemblea,oil_ensemblea = \
    Forward_model_ensemble(modelP,modelS,ensemblepya,rwell,skin,\
                       pwf_producer,mazw,steppi,ensemble.shape[1],DX,pini_alt,\
                           UW,BW,UO,BO,\
                        SWI,SWR,DZ,dt,MAXZ)                              
    print('Read Historical data')
    ...
    ensemblepy = ensemble_pytorch(controlbest,controlbestp,LUB, HUB,mpor,\
                                  hpor,inj_rate,dt,IWSw,\
                         nx,ny,nz,controlbest2.shape[1],input_channel) 
    
    mazw = 0 # Dont smooth the presure field
    _,yycheck,pree,wats,oilss = \
        Forward_model_ensemble(modelP,modelS,ensemblepy,rwell,skin,\
                           pwf_producer,mazw,steppi,controlbest2.shape[1],DX,pini_alt,UW,BW,UO,BO,\
                            SWI,SWR,DZ,dt,MAXZ)      
    
    Plot_RSM_single(yycheck,'Performance.png')
    Plot_petrophysical(controlbest,controlbestp,nx,ny,nz,Low_K1,High_K1)
    
    sio.savemat('RESERVOIR_MODEL.mat', {'permeability':controlbest,\
    'porosity':controlbestp,\
    'Simulated_data_plots':yycheck,'Pressure': pree,\
   'Water_saturation':wats,'Oil_saturation':oilss}) 
    ...
    for kk in range(steppi):  
        ...
        ShowBar(progressBar)
        time.sleep(1)         
        Plot_performance_model(pree[0,:,:,:],wats[0,:,:,:],nx,ny,\
                              'PINN_model_PyTorch.png',UIR,kk,dt,MAXZ,pini_alt)        
    ...
    frames[0].save('Evolution1.gif',format='GIF',\
    ...
    frames[0].save('Evolution_pressure_3D.gif',format='GIF',\
                    append_images=frames[1:],save_all=True,duration=500,loop=0)
    ...
    frames[0].save('Evolution_water_3D.gif',format='GIF',\
                    append_images=frames[1:],save_all=True,duration=500,loop=0)
    ...
    frames[0].save('Evolution_oil_3D.gif',format='GIF',\
                    append_images=frames[1:],save_all=True,duration=500,loop=0)
    ...
    ensemblepy = ensemble_pytorch(yes_best_k,yes_best_p,LUB, HUB,mpor,\
                                  hpor,inj_rate,dt,IWSw,\
                         nx,ny,nz,controlbest2.shape[1],input_channel) 
    
    mazw = 0 # Dont smooth the presure field
    _,yycheck,preebest,watsbest,oilssbest = \
        Forward_model_ensemble(modelP,modelS,ensemblepy,rwell,skin,\
                           pwf_producer,mazw,steppi,controlbest2.shape[1],DX,pini_alt,UW,BW,UO,BO,\
                            SWI,SWR,DZ,dt,MAXZ)      
    ...
    sio.savemat('BEST_RESERVOIR_MODEL.mat', {'permeability':yes_best_k,\
    'porosity':yes_best_p,\
    'Simulated_data_plots':yycheck,'Pressure': preebest,\
   'Water_saturation':watsbest,'Oil_saturation':oilssbest}) 
        
    for kk in range(steppi):   
        progressBar = "\rPlotting Progress: " + ProgressBar(steppi-1, kk-1, steppi-1)
        ShowBar(progressBar)
        time.sleep(1)          
        Plot_performance_model(preebest[0,:,:,:],watsbest[0,:,:,:],nx,ny,\
                              'PINN_model_PyTorch.png',UIR,kk,dt,MAXZ,pini_alt)        
    ...
    cc=((np.sum(((( yycheck) - True_data) ** 2)) )  **(0.5)) \
        /True_data.shape[0]
    print('RMSE of overall best model  = : ' \
        + str(cc) )
    ...
    ensemblepy = ensemble_pytorch(yes_mean_k,yes_mean_p,LUB, HUB,mpor,\
                                  hpor,inj_rate,dt,IWSw,\
                         nx,ny,nz,controlbest2.shape[1],input_channel) 
    ...
    sio.savemat('MEAN_RESERVOIR_MODEL.mat', {'permeability':yes_mean_k,\
    'porosity':yes_mean_p,\
    'Simulated_data_plots':yycheck,'Pressure': preebest,\
   'Water_saturation':watsbest,'Oil_saturation':oilssbest}) 
    ...
    cc=((np.sum(((( yycheck) - True_data) ** 2)) )  **(0.5)) \
        /True_data.shape[0]
    print('RMSE of overall best MAP model  = : ' \
        + str(cc) )
	...
	'PORO_Reali':ensemblep,'P10_Perm':controlbest,'P50_Perm':controljj2,\
	    ...
	    'P90_Poro':controlbadp,'Simulated_data': simDatafinal,\
	'Simulated_data_plots':predMatrix,'Pressures': pressure_ensemble,\
	   'Water_saturation':water_ensemble,'Oil_saturation':oil_ensemble,\
	       'Simulated_data_best_ensemble': simDatafinala,\
	'Simulated_data_plots_best_ensemble':predMatrixa,'Pressures_best_ensemble': \
	    pressure_ensemblea,\
	   'Water_saturation_best_ensemble':water_ensemblea,\
	       'Oil_saturation_best_ensemble':oil_ensemblea}) 
    os.chdir(oldfolder)
    
    ensembleout1=np.hstack([controlbest,controljj2,controlbad,yes_best_k,yes_mean_p])
    ensembleoutp1=np.hstack([controlbestp,controljj2p,controlbadp,yes_best_p,yes_mean_p])
    ...
    ensemblepy = ensemble_pytorch(ensembleout1,ensembleoutp1,LUB, HUB,mpor,\
                                  hpor,inj_rate,dt,IWSw,\
                         nx,ny,nz,ensembleout1.shape[1],input_channel) 
    
    mazw = 0 # Dont smooth the presure field
    _,yzout,pressure_percentile,water_percentile,oil_percentile = \
        Forward_model_ensemble(modelP,modelS,ensemblepy,rwell,skin,\
                           pwf_producer,mazw,steppi,ensembleout1.shape[1],\
                               DX,pini_alt,UW,BW,UO,BO,\
                            SWI,SWR,DZ,dt,MAXZ)      
    ...
    sio.savemat('Posterior_Ensembles_percentile.mat', {'PERM_Reali':ensembleout1,\
    'PORO_Reali':ensembleoutp1,\
    'Simulated_data_plots':yzout,'Pressures': pressure_percentile,\
   'Water_saturation':water_percentile,'Oil_saturation':oil_percentile})     
    ...
    while (snn<1):        
       ...
       # Cini=CDd
        ...
        yyy=np.mean(0.5*((Dd-simDatafinal).T@(np.linalg.inv(CDd))\
                    @(Dd-simDatafinal)),axis=1)
        yyy=yyy.reshape(-1,1)
        yyy=np.nan_to_num(yyy, copy=True, nan=0)
        alpha_star=np.mean(yyy,axis=0)
        ...
        updated_ensemble=Recover_Dictionary_Saarse(Dicclem,\
                                    updated_ensembleksvd,Low_K1,High_K1)
   ...
   'Water_saturation':watsbest,'Oil_saturation':oilssbest}) 
    ...
    mazw = 0 # Dont smooth the presure field
    pertout,yzout,pressure_percentile,water_percentile,oil_percentile = \
        Forward_model_ensemble(modelP,modelS,ensemblepy,rwell,skin,\
                           pwf_producer,mazw,steppi,ensembleout1.shape[1],DX,\
                               pini_alt,UW,BW,UO,BO,\
                            SWI,SWR,DZ,dt,MAXZ)        
    
    
    os.chdir('../HM_RESULTS/PERCENTILE')
    Plot_RSM_percentile(yzout,True_mat,\
                        "P10_P50_P90.png")
   ...
   'Water_saturation':water_percentile,'Oil_saturation':oil_percentile})       
    ...
    print('')
    print('Adaptive Regularised Ensemble Kalman Inversion with Convolution Autoencoder \
Parametrisation\n')
    print('Novel Implementation: Author: Clement Etienam SA Energy/GPU @Nvidia')
    print('Starting the History matching with ', str(Ne) + ' Ensemble members')
    ...
    if (bb==False):
        if use_pretrained ==1:
            print('....Downloading Please hold.........')
            download_file_from_google_drive('1_99X3OrcK4Q8ym325cdJ3z0KX7YiduVw',\
            '../PACKETS/encoder.h5')  
            print('...Downlaod completed.......')
            
            print('....Downloading Please hold.........')
            download_file_from_google_drive('1appHPIKQbOdxkHkwOfLUZMpuheg6ZgKd',\
            '../PACKETS/decoder.h5')  
            print('...Downlaod completed.......') 
            
            print('....Downloading Please hold.........')
            download_file_from_google_drive('1SMZ_hJ_drEVbagjPmcrjxDUB-lVniXPi',\
    ...
    if (bb2==False):
        if use_pretrained ==1:
            print('....Downloading Please hold.........')    
            download_file_from_google_drive('1FWmvrFHwCSuMPxTBqG_9M-CSapOR6hRY',\
            '../PACKETS/encoderp.h5') 
            print('...Downlaod completed.......') 

            
            print('....Downloading Please hold.........')    
            download_file_from_google_drive('19B6RmkYzDih4r3hasuaTSttIF0GfKKuw',\
            '../PACKETS/decoderp.h5') 
            print('...Downlaod completed.......') 

            print('....Downloading Please hold.........')
            download_file_from_google_drive('1SHYUSo2DwFV7OIdfb5SWUoP27dn3nEmI',\
    ...
    ensemblepy = ensemble_pytorch(controlbest,controlbestp,LUB, HUB,mpor,\
                                  hpor,inj_rate,dt,IWSw,\
                         nx,ny,nz,controlbest.shape[1],input_channel) 
    
    mazw = 0 # Dont smooth the presure field
    _,yycheck,pree,wats,oilss = \
        Forward_model_ensemble(modelP,modelS,ensemblepy,rwell,skin,\
                           pwf_producer,mazw,steppi,controlbest2.shape[1],DX,\
                               pini_alt,UW,BW,UO,BO,\
                            SWI,SWR,DZ,dt,MAXZ)     
   ...
   'Water_saturation':watsbest,'Oil_saturation':oilssbest}) 
    ...
    pertout,yzout,pressure_percentile,water_percentile,oil_percentile = \
        Forward_model_ensemble(modelP,modelS,ensemblepy,rwell,skin,\
                           pwf_producer,mazw,steppi,5,DX,pini_alt,UW,BW,UO,BO,\
                            SWI,SWR,DZ,dt,MAXZ)      
    os.chdir('../HM_RESULTS/PERCENTILE')
    Plot_RSM_percentile(yzout,True_mat,\
                        "P10_P50_P90.png")
   ...
   'Water_saturation':water_percentile,'Oil_saturation':oil_percentile})   
    ...
    print('')
    print('Adaptive Regularised Ensemble Kalman Inversion with continous Denoising Autoencoder\n')
    print('Starting the History matching with ', str(Ne) + ' Ensemble members')
    ...
    while (snn<1):        
        ...
        simDatafinal,predMatrix,_,_,_ = \
            Forward_model_ensemble(modelP,modelS,ensemblepy,rwell,skin,\
                               pwf_producer,mazw,steppi,ensemble.shape[1],\
                                   DX,pini_alt,UW,BW,UO,BO,\
                                SWI,SWR,DZ,dt,MAXZ)          
   ...
   'Water_saturation':water_percentile,'Oil_saturation':oil_percentile})       
    ...
    while (snn<1):        
        ...
        ensemble,ensemblep = REKI_ASSIMILATION_LEVELSET(ensemble,\
        ensemblep,simDatafinal,alpha,True_dataa,Ne,\
        pertubations,Experts,Yet,nx,ny,nz,High_K1,Low_K1,High_P,\
        Low_P,CDd)          
    ...
    _,yycheck,pree,wats,oilss = \
        Forward_model_ensemble(modelP,modelS,ensemblepy,rwell,skin,\
                           pwf_producer,mazw,steppi,controlbest.shape[1],\
                               DX,pini_alt,UW,BW,UO,BO,\
                            SWI,SWR,DZ,dt,MAXZ)      
   ...
   'Water_saturation':water_percentile,'Oil_saturation':oil_percentile})       
    ...
    print('--------------------SECTION ADAPTIVE REKI_REKI_LEVELSET ENDED--------------------')
    
elif Technique_REKI==6:
    print('')
    print('Adaptive Regularised Ensemble Kalman Inversion with Normal Score Transformation \n')
    print('Starting the History matching with ', str(Ne) + ' Ensemble members')
    ...
    while (snn<1):        
        ...
        ensemble,ensemblep = REKI_ASSIMILATION_NORMAL_SCORE(ensemble,\
        ensemblep,simDatafinal,alpha,True_dataa,Ne,\
        pertubations,Yet,nx,ny,nz,High_K1,Low_K1,High_P,\
        Low_P,CDd)         
    ...
    mazw = 0 # Dont smooth the presure field
    simDatafinal,predMatrix,pressure_ensmble,water_ensemble,oil_ensemble = \
        Forward_model_ensemble(modelP,modelS,ensemblepy,rwell,skin,\
                           pwf_producer,mazw,steppi,ensemble.shape[1],DX,\
                               pini_alt,UW,BW,UO,BO,\
                            SWI,SWR,DZ,dt,MAXZ)      
    ...
    print('PINO Surrogate Reservoir Simulator Forwarding - ADAPT_REKI_NORMAL_SCORE Model')
   ...
   'Water_saturation':water_percentile,'Oil_saturation':oil_percentile})       
    os.chdir(oldfolder)
    print('--------------------SECTION ADAPTIVE REKI_NORMAL_SCORE ENDED--------------------') 
...
Parametrisation with Generative adverserail network prior\n')
    ...
    if (aa==False):
        if use_pretrained ==1:
            print('....Downloading Please hold.........')
            download_file_from_google_drive('1DSKdVPpGKPeX-h6niGOq0Rf3TUId-AiL',\
            '../PACKETS/generator.h5') 
            print('...Downlaod completed.......')
        else:
    ...
    mazw = 0 # Dont smooth the presure field
    simDatafinal,yycheck,pree,wats,oilss = \
        Forward_model_ensemble(modelP,modelS,ensemblepy,rwell,skin,\
                           pwf_producer,mazw,steppi,controlbest2.shape[1],\
                               DX,pini_alt,UW,BW,UO,BO,\
                            SWI,SWR,DZ,dt,MAXZ)      
   ...
   'Water_saturation':water_percentile,'Oil_saturation':oil_percentile})      
    ...
    print('')
    print('Adaptive Regularised Ensemble Kalman Inversion with Generative\
adverserail network prior for permeability field alone\n')
   ...
   'Water_saturation':water_percentile,'Oil_saturation':oil_percentile})       
    ...
    if (bb==False):        
        if use_pretrained == 1:
             ...
             print('....Downloading Please hold.........')   
             download_file_from_google_drive('1rv152M5sMIhluyjGAVdpviVVzCVXHD5j',\
            ...
            kmeans = MiniBatchKMeans(n_clusters=Ne,
                                      random_state=0,
                                     max_iter=2000)
   ...
   'Water_saturation':water_percentile,'Oil_saturation':oil_percentile})          
    ...
    print('')
    print('Adaptive Regularised Ensemble Kalman Inversion with Variational Convolution Autoencoder \
Parametrisation\n')
    print('Novel Implementation: Author: Clement Etienam SA Energy/GPU @Nvidia')
    print('Starting the History matching with ', str(Ne) + ' Ensemble members')
    print('-------------------------learn Variational Autoencoder------------------------')
    ...
    while (snn<1):        
        ...
        if ii==0:
            ...
            encodedp = Get_LatentV(ensemblep,N_ens,nx,ny,nz,latent_dim,vaep,High_P,2)
        else:
            #sgsim=ensemble
            encoded = updated_ensemble
            encodedp = updated_ensemblep
        ...
        ensemble = Recover_imageV(updated_ensemble,Ne,nx,ny,nz,latent_dim,vae,High_K1,1)
        ensemble[ensemble<=Low_K1]=Low_K1
        ensemble[ensemble>=High_K1]=High_K1
        
        ensemblep = Recover_imageV(updated_ensemblep,Ne,nx,ny,nz,latent_dim,vaep,High_P,2)
   ...
   'Water_saturation':watsbest,'Oil_saturation':oilssbest}) 
    ...
    pertout,yzout,pressure_percentile,water_percentile,oil_percentile = \
        Forward_model_ensemble(modelP,modelS,ensemblepy,rwell,skin,\
                           pwf_producer,mazw,steppi,ensembleout1.shape[1],DX,pini_alt,UW,BW,UO,BO,\
                            SWI,SWR,DZ,dt,MAXZ)         
    os.chdir('../HM_RESULTS/PERCENTILE')
    Plot_RSM_percentile(yzout,True_mat,\
                        "P10_P50_P90.png")
...
if Technique_REKI==11:
    print('')
    print('Adaptive Regularised Ensemble Kalman Inversion with Mixture of Experts prior\n')
...
Gaussian Process Experts. arXiv preprint arXiv:2006.13309, 2020.\n')
    ...
    while (snn<1):        
        ...
        for jj in range(N_ens):
            #print(str(jj +1) + ' | ' +  str (N_ens))
            spatout = Prediction_CCR_Ensemble('Realizationn_',oldfolder,jj,\
                                              ensembletemp[:,jj],nclusters)
   ...
   'Water_saturation':water_percentile,'Oil_saturation':oil_percentile})       
    ...
    print('--------------------SECTION ADAPTIVE REKI CCR ENDED----------------------------') 
    ...
    sizedct = None
    while True:
        sizedct=int(input('Enter the percentage of required DCT \
    coeficients (15%-30%): '))
    ...
    while (snn<1):        
       ...
       # Cini=CDd
        ...
        updated_ensemble = idct22(updated_ensembleksvd,Ne,nx,ny,nz,\
                                size1,size2)  

        updated_ensemblep = idct22(updated_ensembleksvdp,Ne,nx,ny,nz,\
                                size1,size2)  
   ...
   'Water_saturation':water_percentile,'Oil_saturation':oil_percentile})       
    ...
    print('')
    print('Adaptive Regularised Ensemble Kalman Inversion with Logit \
post-processing\n')
    print('Starting the History matching with ', str(Ne) + ' Ensemble members')
    Low_precision = 1e-10    
    ensemble=ini_ensemble
   ...
   'Water_saturation':water_percentile,'Oil_saturation':oil_percentile})     
    ...
    os.chdir(oldfolder)         
    print('--------------------SECTION ADAPTIVE REKI LOGIT ENDED----------------------------')  
    
elif Technique_REKI==14:
    print('')
    print('Adaptive Regularised Ensemble Kalman Inversion with Diffusion model Parametrisation\n')
    print('Novel Implementation: Author: Clement Etienam SA Energy/GPU @Nvidia')
    print('Starting the History matching with ', str(Ne) + ' Ensemble members')
    ...
    if aa==False:  
        ...
        mat = sio.loadmat(filename)
        ini_ensemble=mat['Z']    
        ini_ensemble = rescale_linear(rescale_linear(ini_ensemble, Low_K1, High_K1), 0, 1)    
        val = ini_ensemble.shape[1]
        
        
        if use_pretrained ==1:
            print('....Downloading Please hold.........')    
            download_file_from_google_drive('1bj-gKaRAKDPvMykF7LNTHsYfIpRB1sLD',\
            '../PACKETS/diffusion_model.pth')
            print('...Downlaod completed.......')
    ...
    ini_ensemble = rescale_linear(pytorch_to_inverse(Ne,sampled_images,nx,ny,nz),\
                                  Low_K1,High_K1)
    ...
    while (snn<1):        
        ...
        updated_ensemble = inverse_to_pytorch(Ne,encoded,nx,ny,nz,device,make_up) #take back to pytorch format 
        ...
        ensemble = rescale_linear(pytorch_to_inverse(Ne,sampled_images,nx,ny,nz),Low_K1,High_K1)
   ...
   'Water_saturation':water_percentile,'Oil_saturation':oil_percentile})       
    ...
    print('--------------------SECTION ADAPTIVE REKI_DIFFUSION ENDED-------------------') 
...
if Technique_REKI==1:
    ...
    comment = 'ADAPT_REKI_DA  (Adaptive Ensemble Kalman Inversion + Denoising Autoencoder)\n'
elif Technique_REKI ==5:
    comment = 'ADAPT_REKI_LEVELSET  (Adaptive Ensemble Kalman Inversion + Level Set)\n'
elif Technique_REKI ==6:
    comment = 'ADAPT_REKI_NORMAL_SCORE  (Adaptive Ensemble Kalman Inversion + Normal Score)\n'
elif Technique_REKI ==7:
    ...
    comment = 'ADAPT_REKI_DCT  (Adaptive Ensemble Kalman Inversion with DCT parametrisation\n'
elif Technique_REKI ==13:
