...
def Black_oil2(UIR,pini_alt,LUB, HUB,aay, bby,SWI,SWR,UW,BW,UO,BO, MAXZ,
             nx,ny,input_var,device,myloss):
    ...
    # water_sat = sat
     ...
     Mo = torch.div(torch.square(torch.sub(torch.ones(S.shape,\
                             device = u.device),S)),(UO*BO) )  
     ...
     for m in range (sat.shape[0]): #Batch
         ...
         dudx_fdma = dx(inn_now, dx=dxf, channel=0, dim=0, order=1, padding="replication")
         dudy_fdma = dx(inn_now, dx=dxf, channel=0, dim=1, order=1, padding="replication") 
     ...
     yes1 = ((Tx - est_sigma1) ** 2 
        + (Ty - est_sigma2) ** 2).mean() 
     ...
     yes1w = ((Txw - est_sigma1) ** 2 
        + (Tyw - est_sigma2) ** 2).mean() 
...
def Black_oil(UIR,pini_alt,LUB, HUB,aay, bby,SWI,SWR,UW,BW,UO,BO, MAXZ,
             nx,ny,input_var,device,myloss):
    ...
    # water_sat = sat
     ...
     for m in range (sat.shape[0]): #Batch
         inn_now = pressure[m,:,:,:][:,None,:,:]
         dudx_fdma = ddx(inn_now, dx=dxf, channel=0, dim=0, order=1, padding="replication")
         dudy_fdma = ddx(inn_now, dx=dxf, channel=0, dim=1, order=1, padding="replication") 
         gulpa.append(dudx_fdma)
         gulp2a.append(dudy_fdma)    
     ...
     + (dcdy * dudy_fdm)
     + (a1 * dduddy_fdm) )
     darcy_pressure = torch.sum(torch.abs((finuse.reshape(sat.shape[0],-1) - right.reshape(sat.shape[0],-1)))/sat.shape[0])
     # Zero outer boundary
...
def Forward_model_pytorch(modelP,modelS,x_true,rwell,skin,\
                           pwf_producer,mazw,steppi,Ne,DX,pini_alt,UW,BW,UO,BO,\
                              SWI,SWR,DZ,dt,MAXZ,UIR,LUB,HUB,aay,bby,nx,ny,device,muloss):
#### ===================================================================== ####
#                     PINO RESERVOIR SIMULATOR
#                      
#### ===================================================================== ####
    ...
    if surrogate ==4:
        f_loss2, f_water2 = Black_oil2(UIR,pini_alt,LUB, HUB,aay, bby,SWI,SWR,UW,BW,UO,BO, MAXZ,
                     nx,ny,input_var,device,myloss)        
    else:
        f_loss2, f_water2 =  Black_oil(UIR,pini_alt,LUB, HUB,aay, bby,SWI,SWR,UW,BW,UO,BO, MAXZ,
                         nx,ny,input_var,device,myloss)    
    ...
    v_min, v_max = torch.min(inn["perm"][amm,0,:,:]), torch.max(inn["perm"][amm,0,:,:])
    new_min, new_max = aay, bby     
    m = (new_max - new_min) / (v_max - v_min)
    b = new_min - m *v_min
    ...
    for kk in range(steppi):
        ...
        average_pressure = (torch.index_select(Ptito,0,producer_location) )\
            * pini_alt
        
        p_inj = (torch.index_select(Ptito,0,Injector_location) )\
            * pini_alt
...
def Peaceman_well(inn,ooutp,oouts,MAXZ,mazw,s1,LUB,HUB,aay,bby,DX,steppi,\
                pini_alt,SWI,SWR,UW,BW,DZ,rwell,skin,UO,BO,pwf_producer,dt):  
    ...
    PERM = rescale_linear_pytorch_numpy(np.reshape(inn["perm"].detach().cpu().numpy(),(-1,),'F'),\
                                        LUB,HUB, aay, bby) 
    kuse_inj = PERM[Injector_location]
...
def log_prob(x):     
     ...
     mazw = 0 # Dont smooth the presure field
     simDatafinal,_,_,_,_ = Forward_model_ensemble(modelP,modelS,ensemblepy,rwell,skin,\
                       pwf_producer,mazw,steppi,1,DX,pini_alt,UW,BW,UO,BO,\
                        SWI,SWR,DZ,dt,MAXZ)            
...
if DEFAULT==1:
    ...
    choicey = None
    while True:
        choicey=int(input('Denoise the update :\n1=Denoising autoencoder\n2=Sal and pepper\n'))
...
print('')
print('LBFGS with Denoising Autoencoder\
post-processing\n')
...
for na in range(2):  
    for epoch in range(start_epoch, epochs + 1): 
        ...
        mazw = 0 # Dont smooth the presure field
        simDatafinal,fpp,fss = \
            Forward_model_pytorch(modelP,modelS,ensemblepy,rwell,skin,\
                                pwf_producer,mazw,steppi,1,DX,\
                                    pini_alt,UW,BW,UO,BO,\
                                SWI,SWR,DZ,dt,MAXZ,UIR,LUB,HUB,aay,bby,nx,ny,device,muloss)           
        ...
        print(   'TRAINING')        
        if aha<ahnewa:
            print('   INVERSE PROBLEM COMMENT : Loss increased by ' + \
                  str(abs(aha-ahnewa)))
        elif aha>ahnewa:
            print('   INVERSE PROBLEM COMMENT : Loss decreased by ' + \
                  str(abs(aha-ahnewa)) )
        else:
...
optimizer1 = torch.optim.LBFGS([clement] , lr,
                              max_iter = steps,
                              max_eval = None,
                              tolerance_grad = 1e-7,
...
ensemblea,ensemblepa=honour2(Best_phi,\
                         Best_K,nx,ny,nz,N_ens,\
                             High_K1,Low_K1,High_P,Low_P)   
...
_,yycheck,pree,wats,oilss = \
    Forward_model_ensemble(modelP,modelS,ensemblepy,rwell,skin,\
                       pwf_producer,mazw,steppi,controlbest.shape[1],DX,pini_alt,UW,BW,UO,BO,\
                        SWI,SWR,DZ,dt,MAXZ)      
