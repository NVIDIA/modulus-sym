...
#### ===================================================================== ####
    ...
    for amm in range(Ne):
        ...
        Injector_location = np.where(Reinvent(inn["Qw"][amm,0,:,:,:].detach().cpu().numpy()).ravel()>0)[0]
        producer_location = np.where(Reinvent(inn["Q"][amm,0,:,:,:].detach().cpu().numpy()).ravel()<0)[0]

        PERM = rescale_linear_pytorch_numpy(np.reshape(Reinvent(inn["perm"][amm,0,:,:,:].detach().cpu().numpy()),(-1,),\
                                    'F'),LUB,HUB, aay, bby) 
        ...
        for kk in range(steppi):
            ...
            average_pressure = (Ptito.ravel()[producer_location] ) * pini_alt#np.mean(Ptito.ravel()) * pini_alt
...
def ensemble_pytorch(param_perm,param_poro,LUB, HUB,mpor, hpor,inj_rate,dt,IWSw,\
    ...
    for kk in range(Ne):        
        ...
        for my in range(nz):
            ...
            ini_ensemble3[kk,0,my,:,:] = quse_water[:,:,my]/UIR#Water injection source
            ...
            ini_ensemble7[kk,0,my,:,:] = IWSw * np.ones((nx,ny)) #INitial water saturation        
...
def Variational_Autoencoder(latent_dim,nx,ny,nz,High_K,Low_K,mem):
    ...
    x = tensorflow.keras.layers.Conv2D(32, 3, activation="gelu", strides=2, padding="same")(encoder_inputs)
    x = tensorflow.keras.layers.Conv2D(64, 3, activation="gelu", strides=2, padding="same")(x)
    ...
    x = tensorflow.keras.layers.Conv2DTranspose(64, 3, activation="gelu", strides=2, padding="same")(x)
    x = tensorflow.keras.layers.Conv2DTranspose(32, 3, activation="gelu", strides=2, padding="same")(x)
...
modelP = FNOArch([Key("perm", size=1),Key("Q", size=1),Key("Qw", size=1),\
         Key("Phi", size=1),Key("Time", size=1),Key("Pini", size=1),\
            Key("Swini", size=1) ], fno_modes = 16, dimension=3,padding=13,nr_fno_layers=4, decoder_net=decoder1)   
...
modelS = FNOArch([Key("perm", size=1),Key("Q", size=1),Key("Qw", size=1),\
         Key("Phi", size=1),Key("Time", size=1),Key("Pini", size=1),\
            Key("Swini", size=1) ], fno_modes = 16, dimension=3,padding=13,nr_fno_layers=4, decoder_net=decoder2)  
...
if surrogate ==1:
    ...
    if (bb==False): 
        ...
        download_file_from_google_drive('1WcLz50Iz5nlBtAYYdAjpigwc7qCtxG9W',\
    ...
    if (bba==False):  
        print('....Downloading Please hold.........')               
        download_file_from_google_drive('1V-7wSyaV7Fd_tThedqlL-q7861p6ZzHj',\
    ...
    if (bb==False):      
        print('....Downloading Please hold.........')      
        download_file_from_google_drive('1YxNkCTEWCUDyYbztFSTnEaRV2h3AR6yT',\
    ...
    if (bba==False):      
        print('....Downloading Please hold.........')     
        download_file_from_google_drive('1L1b9Jhaz-jAgFUGASqf5QY6onsiRf9VZ',\
...
wells = np.array([1,24,1,1,1,1,31,1,1,31,31,1,7,9,2,
...
if (bba==False):
    print('....Downloading Please hold.........')
    download_file_from_google_drive('1VSy2m3ocUkZnhCsorbkhcJB5ADrPxzIp',\
...
if DEFAULT ==1:
    ...
    if (bb==False):      
        print('....Downloading Please hold.........')    
        download_file_from_google_drive('1wYyREUcpp0qLhbRItG5RMPeRMxVtntDi',\
        ...
        if afresh==1:
            ...
            if Ne <=2000:
                ...
                if (bb==False):            
                    ...
                    download_file_from_google_drive('1KZvxypUSsjpkLogGm__56-bckEee3VJh',\
...
post-processing\n')
    ...
    if (Geostats==1):
        ...
        if (bb==False) :
            if use_pretrained == 1: 
                print('....Downloading Please hold.........')
                download_file_from_google_drive('1S71PoGY3MY3lBuuqBahstXZTslZsvXTs',\
        ...
        if (bb2==False) : 
            ...
            if use_pretrained == 1: 
                print('....Downloading Please hold.........')
                download_file_from_google_drive('1HZ-LeVLWO9ZsDEde4LG11aOxKOt4uBVn',\
...
Parametrisation\n')
    ...
    if (bb==False):
        if use_pretrained ==1:
            print('....Downloading Please hold.........')
            download_file_from_google_drive('1AEZPXAoJaC88dY9T-FFhLwJ_9U-Pl3tq',\
            '../PACKETS/encoder.h5')  
            print('...Downlaod completed.......')
            
            print('....Downloading Please hold.........')
            download_file_from_google_drive('1u4Vo7XdyZQq4Z0_jNsoqG4QNWslAwyBv',\
            '../PACKETS/decoder.h5')  
            print('...Downlaod completed.......') 
            
            print('....Downloading Please hold.........')
            download_file_from_google_drive('16Be2bIUWhiq8RBT48-baIHGU47XSnixS',\
    ...
    if (bb2==False):
        if use_pretrained ==1:
            print('....Downloading Please hold.........')    
            download_file_from_google_drive('1Q2oi6TmyjE-KkShhyqqi2nZRRa6I8OYB',\
            '../PACKETS/encoderp.h5') 
            print('...Downlaod completed.......') 

            
            print('....Downloading Please hold.........')    
            download_file_from_google_drive('1Y4sQxR4Uj_OO4QjwNUocOe66Zmtu0JsK',\
            '../PACKETS/decoderp.h5') 
            print('...Downlaod completed.......') 

            print('....Downloading Please hold.........')
            download_file_from_google_drive('1fUUSBeXXZLP9PhME5oE_3yG_5ETAzIMk',\
