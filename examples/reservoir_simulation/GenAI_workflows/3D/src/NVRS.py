...
def v_cycle(A, b, x, levels,tol, smoothing_steps=2):
    ...
    for level in range(levels):
        if level == levels - 1:
            ...
            coarse_x = v_cycle(coarse_A, coarse_r, cp.zeros_like(coarse_r), levels-1,tol)
        ...
        while I < 2**IT:
            ...
            while (dsn > 0.001) and (it < 10):
                ...
                if method2 == 1: #GMRES
                    ...
                    # print(dG.shape)
                    G= cp.ravel(G)
                    ds = v_cycle(-dG, G, x = cp.zeros_like(G), levels=2, tol = 1e-5, smoothing_steps=2)   
...
def Get_actual_few(ini,nx,ny,nz,max_t,Dx,Dy,Dz,BO,BW,CFL,timmee,MAXZ,factorr,steppi,\
    ...
    for kk in range(Ne):
        ...
        A_out = Reservoir_Simulator(a,at2,quse.ravel(),quse_water.ravel(),nx,\
                ny,nz,factorr,max_t,Dx,Dy,Dz,BO,BW,CFL,timmee,MAXZ,PB,PATM,CFO,\
