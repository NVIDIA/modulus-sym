...
def Geta_all(folder,nx,ny,nz,effective,oldfolder,check,steppi,steppi_indices):
    ...
    for pr_slice, sw_slice, sg_slice,so_slice in zip(filtered_pressure, 
                    filtered_swat, filtered_sgas,filtered_soil):
        for state_var, all_slices in zip([pr_slice, sw_slice, sg_slice, so_slice], 
...
Trainmoe = 2
print('-----------------------------------------------------------------')
print('Using Cluster Classify Regress (CCR) for peacemann model -Hard Prediction          ')
print('')
...
folder = path_out
Pr,sw,sg,so,tt,_ = Geta_all(folder,nx,ny,nz,effective,oldfolder2,check,
...
_,out_fcn_true = Get_data_FFNN1(folder,oldfolder2,N,pressure_true,Sgas_true,
                                Swater_true,Soil_true,permeability,Time,steppi,steppi_indices)
...
fno_oil = FNOArch([Key("perm", size=1),Key("Phi", size=1), Key("Pini", size=1),\
            Key("Swini", size=1),Key("fault", size=1) ],\
dimension=3, decoder_net=decoder3) 
...
_,ouut_peacemann,pressure,Swater,Sgas,Soil = Forward_model_ensemble(Ne,inn,steppi,min_inn_fcn,max_inn_fcn,
                 ...
                 ,fno_water,fno_gas,fno_oil,device,fno_peacemann,
                 min_out_fcn,max_out_fcn,Time,effectiveuse,Trainmoe,num_cores,pred_type,
                 oldfolder,degg,experts)
...
if modulus_time < flow_time:
    ...
    plt.text(0.5, 550, f'Speedup: {speedup}x', ha='center', fontsize=12, fontweight='bold', color='blue')
...
for k in range(22):
    ...
    plt.plot(timezz,P10[:,k], color = 'blue', lw = '2', \
             label ='PINO -CCR(hard)')
    plt.plot(timezz,P90[:,k], color = 'orange', lw = '2', \
         label ='PINO - FNO')
...
for k in range(22):
    ...
    plt.plot(timezz,P10[:,k+22], color = 'blue', lw = '2', \
             label ='PINO -CCR(hard)')
    plt.plot(timezz,P90[:,k+22], color = 'orange', lw = '2', \
         label ='PINO - FNO')              
...
for k in range(22):
    ...
    plt.plot(timezz,P10[:,k+44], color = 'blue', lw = '2', \
             label ='PINO -CCR(hard)')
    plt.plot(timezz,P90[:,k+44], color = 'orange', lw = '2', \
         label ='PINO - FNO')             
...
cc1 = ((np.sum(((( CCRhard) - True_data) ** 2)) )  **(0.5)) \
    /True_data.shape[0]
print('RMSE of PINO - CCR (hard prediction) reservoir model  =  ' \
    + str(cc1) )  
...
cc3 = ((np.sum(((( FNO) - True_data) ** 2)) )  **(0.5)) \
    /True_data.shape[0]
print('RMSE of  PINO - FNO reservoir model  =  ' \
    + str(cc3) )    
