...
def load_FNO_dataset2(path, input_keys, output_keys1,output_keys2,output_keys3,output_keys4, n_examples=None):
    "Loads a FNO dataset"
    ...
    for d, keys in [(invar, input_keys), (outvar1, output_keys1),
                (outvar2, output_keys2), (outvar3, output_keys3), (outvar4, output_keys4) ]:
        for k in keys:
...
def run(cfg: ModulusConfig) -> None:
    ...
    data = {'perm': cPerm,'Phi': cPhi,
        'Pini': cPini,'Swini': cSini,'pressure': cPress,'water_sat': cSat,
        'gas_sat': cSatg,'fault':cfault,
        'oil_sat': cSato}
    ...
    cSini,'pressure': cPress,'water_sat': cSat,'gas_sat': cSatg,'fault':cfault,'oil_sat': cSato}
    ...
    invar_train, outvar_train1,outvar_train2,outvar_train3,outvar_train4 = load_FNO_dataset2(
    ...
    invar_test, outvar_test1,outvar_test2,outvar_test3,outvar_test4 = load_FNO_dataset2(
    ...
    outvar_train2["waterd"] = np.zeros_like(
        outvar_train2["water_sat"]
    )

    outvar_train3["gasd"] = np.zeros_like(
        outvar_train3["gas_sat"]
    )

    outvar_train4["oild"] = np.zeros_like(
        outvar_train3["oil_sat"]
    )      
