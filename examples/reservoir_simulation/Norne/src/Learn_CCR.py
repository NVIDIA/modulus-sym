...
def Performance_plot_cost(CCR,Trued,stringg,training_master,oldfolder):
    ...
    for jj in range(Trued.shape[1]):
        ...
        sc=plt.scatter(np.ravel(clementanswer2),np.ravel(outputtest2),\
                       c=np.ravel(outputtest2), vmin=vmin, vmax=vmax, s=35, cmap=palette)
        plt.colorbar(sc)
...
(inpuutx2, outpuutx2, test_size=0.01) #train_size=2000)
...
with executor_run:
    bigs = Parallel(n_jobs=num_cores,backend='multiprocessing', verbose=50)(delayed(
    startit)\
...
with executor_run1:
    clemes=Parallel(n_jobs=num_cores,backend='multiprocessing', verbose=50)(delayed(
...
(outputpredenergy,y_test2,"Machine_Energy_perform",trainingmaster,\
 oldfolder)    
