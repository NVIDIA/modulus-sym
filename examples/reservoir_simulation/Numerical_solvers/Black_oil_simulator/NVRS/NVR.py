...
def Upstream_3PHASE(nx,ny,nz,S,Soil,UW,UO,UG,BW,BO,BG,RS,SWI,SWR,Vol,qinn,qinnoil,V,Tt,porosity,SWOW,SWOG):
    ...
    for t in range(Nts):
        ...
        mw,mo,mg,_,_,_ = RelPerm3(S,Soil, UW, UO, UG, BW, BO, BG, SWI, SWR, nx, ny, nz,SWOW,SWOG)
...
def NewtRaph2(nx,ny,nz,porosity,Vol,S,Soil,V,qinn,qinnoil,Tt,UW,UO,UG,SWI,SWR,method2,BW,BO,BG,RS,SWOW,SWOG):
    ...
    while conv==0:
        ...
        while I < 2**IT:
            ...
            while (dsn > 0.01) and (it < 5):
                ...
                Mw,Mo,Mg,dMw,dMo,dMg = RelPerm3(S,Soil, UW, UO, UG, BW, BO, BG, SWI, SWR, nx, ny, nz,SWOW,SWOG)
...
def Reservoir_Simulator2(Kuse,porosity,quse,quse_water,quse_oil,nx,ny,nz,factorr,max_t,\
 Dx,Dy,Dz,BO,BW,BG,RS,CFL,timmee,MAXZ,PB,PATM,CFO,IWSw,IWSo,method,steppi,SWI,SWR,UW,UO,UG,\
    ...
    for t in range(tc2.shape[0]-1):
        ...
        if CFL == 1:
            S,Soil = Upstream_3PHASE(nx,ny,nz,S,Soil,UW,UO,UG,BW,BO,BG,RS,SWI,SWR,Vol,quse_water,quse_oil,V,dt,porosity,SWOW,SWOG)
        else:
            for ts in range(step2):

                S,Soil = NewtRaph2(nx,ny,nz,porosity,Vol,S,Soil,V,quse_water,quse_oil,\
                           St/float(step2),UW,UO,UG,SWI,SWR,method,BW,BO,BG,RS,SWOW,SWOG)            
