...
Finite Volume Black Oil Reservoir Simulator
...
def inference_single2(ini,inip,nx,ny,nz,max_t,Dx,Dy,Dz,BO,BW,BG,RS,CFL,timmee,MAXZ,\
                   ...
                   aay,bby,mpor,hpor,dt,IWSw,IWSg,PB,PATM,CFO,method,\
                    SWI,SWR,UW,UO,UG,step2,pini_alt,INJ,PROD,opennI,opennP,SWOW,SWOG):
    ...
    for kk in range(Ne):
        ...
        A_out = Reservoir_Simulator2(a,at2,quse.ravel(),\
                quse_water.ravel(),quse_oil.ravel(),nx,ny,nz,factorr,max_t,\
                    ...
                    IWSw,IWSg,method,steppi,SWI,SWR,UW,UO,UG,step2,pini_alt,SWOW,SWOG)
        if nz ==1:  
            ct1[0,:,:] =  at1[:,:,0] # permeability
            ct1[1,:,:] = quse1[:,:,0]#/UIR # Overall f
            ct1[2,:,:] = Ainj[:,:,0]#/UIR# f for water injection
        ...
        if N_components ==2:
           ...
           'Water_saturation':data_use1[0,steppi:,:,:][:,:,None],\
            ...
            X_data1, data_use1 = inference_single2(ini,inip,nx,ny,nz,max_t,DX,DY,DZ,BO,BW,BG,RS,CFL\
                                                 ,timmee,MAXZ,factorr,steppi,\
                               aay,bby,mpor,hpor,dt,IWSw,IWSg,PB,PATM,CFO,method,\
                                SWI,SWR,UW,UO,UG,step2,pini_alt,injectors,producers,opennI,opennP,SWOW,SWOG) 
            os.chdir(path_out)
        ...
        if N_components ==2:
            if make_animation:
                ...
                if nz ==1: 
                    ...
                    Parallel(n_jobs=-1)(delayed(Plot_performance)(data_use1[0,:,:,:],nx,ny,'new',kk,dt,
                    MAXZ,steppi,wells)for kk in range(steppi))
                    

                    Parallel(n_jobs=-1)(delayed(plot3d2)(data_use1[0,kk,:,:][:,:,None]/max((data_use1[0,kk,:,:][:,:,None]).ravel()),
                    nx,ny,nz,kk,dt,
                    MAXZ,'unie_pressure','Pressure',
                    max((data_use1[0,kk,:,:][:,:,None]).ravel()),min((data_use1[0,kk,:,:][:,:,None]).ravel()),
                    injectors,producers)for kk in range(steppi))


                    

                    Parallel(n_jobs=-1)(delayed(plot3d2)(data_use1[0,kk + steppi,:,:][:,:,None],nx,ny,nz,kk,dt,
                    MAXZ,'unie_water','water_sat',max((data_use1[0,kk + steppi,:,:][:,:,None]).ravel()),
                    min((data_use1[0,kk + steppi,:,:][:,:,None]).ravel()),injectors,producers)for kk in range(steppi)) 
                    

                    Parallel(n_jobs=-1)(delayed(plot3d2)((( 1- data_use1[0,kk + steppi,:,:][:,:,None])),nx,ny,nz,kk,dt,MAXZ,'unie_oil','oil_sat',
                    max(( 1- data_use1[0,kk + steppi,:,:][:,:,None]).ravel()),
                    min(( 1- data_use1[0,kk + steppi,:,:][:,:,None]).ravel()),injectors,producers)for kk in range(steppi))
                        
                    progressBar = "\rPlotting Progress: " + ProgressBar(Runs-1, Runs-1, Runs-1)
                    ShowBar(progressBar)
                    time.sleep(1)      
                    ...
                    Parallel(n_jobs=-1)(delayed(plot3d2)(data_use1[0,kk,:,:,:][:, :, ::-1]/max((data_use1[0,kk,:,:,:][:, :, ::-1]).ravel()),
                    nx,ny,nz,kk,dt,MAXZ,'unie_pressure','Pressure',max((data_use1[0,kk,:,:,:][:, :, ::-1]).ravel()),
                    min((data_use1[0,kk,:,:,:][:, :, ::-1]).ravel()),injectors,producers)for kk in range(steppi))
                    

                    Parallel(n_jobs=-1)(delayed(plot3d2)(data_use1[0,kk + steppi,:,:,:][:, :, ::-1],nx,ny,nz,kk,dt,
                    MAXZ,'unie_water','water_sat',max((data_use1[0,kk + steppi,:,:,:][:, :, ::-1]).ravel()),
                    min((data_use1[0,kk + steppi,:,:,:][:, :, ::-1]).ravel()),injectors,producers)for kk in range(steppi)) 
                                        

                    Parallel(n_jobs=-1)(delayed(plot3d2)(1- data_use1[0,kk + steppi,:,:,:][:, :, ::-1],nx,ny,nz,kk,dt,
                    MAXZ,'unie_oil','oil_sat',max((1- data_use1[0,kk + steppi,:,:,:][:, :, ::-1]).ravel()),
                    min((1- data_use1[0,kk + steppi,:,:,:][:, :, ::-1]).ravel()),injectors,producers)for kk in range(steppi)) 
            ...
            if make_gassman: 
                ...
                if nz ==1: 
                    ...
                    Parallel(n_jobs=-1)(delayed(Plot_impedance)((Gassmann(cp.asarray(np.reshape(inip,(nx,ny,nz),'F'),) ,
                    cp.asarray(data_use1[0,kk,:,:][:,:,None]), 
                    cp.asarray((( 1- data_use1[0,kk + steppi,:,:][:,:,None]))),nx,ny,nz)).get(),
                    nx,ny,'newI',kk,dt,MAXZ,steppi,injectors,producers)for kk in range(steppi)) 
                    ...
                    frames1[0].save('Evolution2_impededance.gif',format='GIF',\
                                    append_images=frames1[1:],save_all=True,duration=500,loop=0) 
                    ...
                    Parallel(n_jobs=-1)(delayed(Plot_impedance)((Gassmann(cp.asarray(np.reshape(inip,(nx,ny,nz),'F'),) ,
                    cp.asarray(data_use1[0,kk,:,:,:]), 
                    cp.asarray((( 1- data_use1[0,kk + steppi,:,:,:]))),nx,ny,nz)).get(),
                    nx,ny,'newI',kk,dt,MAXZ,steppi,injectors,producers)for kk in range(steppi))                                     
                    ...
                    frames[0].save('Evolution2_impededance.gif',format='GIF',\
                                    append_images=frames[1:],save_all=True,duration=500,loop=0)
            ...
            msg = "Seismic plots took: %s secs (Wall clock time)" % timedelta(seconds=round(elapsed_time_secs))
            print(msg)
            ...
            if make_csv:
                ...
                if nz ==1:
                    seeTrue = Peaceman_well(X_data1,data_use1[:,:steppi,:,:],\
                                    ...
                                    pini_alt,SWI,SWR,UW,BW,DZ,rwell,skin,UO,BO,\
                                pwf_producer,dt,N_inj,N_pr,nz,NecessaryI,NecessaryP)
                else:
                    seeTrue = Peaceman_well(X_data1,data_use1[:,:steppi,:,:,:],\
                                            data_use1[:,steppi:,:,:,:],MAXZ,0,1e1,DX,steppi,\
                                    pini_alt,SWI,SWR,UW,BW,DZ,rwell,skin,UO,BO,\
            ...
            if make_animation:
                ...
                if nz ==1:  
                    ...
                    Parallel(n_jobs=-1)(delayed(Plot_performance2)(data_use1[0,:,:,:],nx,ny,'new',kk,dt,
                    MAXZ,steppi,wells)for kk in range(steppi))
                               

                    Parallel(n_jobs=-1)(delayed(plot3d2)(data_use1[0,kk,:,:][:,:,None]/max((data_use1[0,kk,:,:][:,:,None]).ravel()),
                    nx,ny,nz,kk,dt,
                    MAXZ,'unie_pressure','Pressure',max((data_use1[0,kk,:,:][:,:,None]).ravel()),
                    min((data_use1[0,kk,:,:][:,:,None]).ravel()),injectors,producers)for kk in range(steppi))
                    

                    

                    Parallel(n_jobs=-1)(delayed(plot3d2)(data_use1[0,kk + steppi,:,:][:,:,None],nx,ny,nz,kk,dt,
                    MAXZ,'unie_water','water_sat',
                    max((data_use1[0,kk + steppi,:,:][:,:,None]).ravel()),
                    min((data_use1[0,kk + steppi,:,:][:,:,None]).ravel()),injectors,producers)for kk in range(steppi)) 


                    Parallel(n_jobs=-1)(delayed(plot3d2)(data_use1[0,kk + 2*steppi,:,:][:,:,None],nx,ny,nz,kk,dt,
                    MAXZ,'unie_oil','oil_sat',max((data_use1[0,kk + 2*steppi,:,:][:,:,None]).ravel()),
                    min((data_use1[0,kk + 2*steppi,:,:][:,:,None]).ravel()),injectors,producers)for kk in range(steppi))                     
                    

                    Parallel(n_jobs=-1)(delayed(plot3d2)((( data_use1[0,kk + 3*steppi,:,:][:,:,None])),nx,ny,nz,kk,
                    dt,MAXZ,'unie_gas','gas_sat',max((( data_use1[0,kk + 3*steppi,:,:][:,:,None])).ravel()),
                    min((( data_use1[0,kk + 3*steppi,:,:][:,:,None])).ravel()),injectors,producers)for kk in range(steppi)) 
                    ...
                    Parallel(n_jobs=-1)(delayed(plot3d2)(data_use1[0,kk,:,:,:][:, :, ::-1]/max((data_use1[0,kk,:,:,:][:, :, ::-1]).ravel()),
                    nx,ny,nz,kk,dt,
                    MAXZ,'unie_pressure','Pressure',max((data_use1[0,kk,:,:,:][:, :, ::-1]).ravel()),
                    min((data_use1[0,kk,:,:,:][:, :, ::-1]).ravel()),injectors,producers)for kk in range(steppi))
                    ...
                    Parallel(n_jobs=-1)(delayed(plot3d2)(data_use1[0,kk + 2*steppi,:,:,:][:, :, ::-1],nx,ny,nz,kk,dt,
                    MAXZ,'unie_oil','oil_sat',max((data_use1[0,kk + 2*steppi,:,:,:][:, :, ::-1]).ravel()),
                    min((data_use1[0,kk + 2*steppi,:,:,:][:, :, ::-1]).ravel()),injectors,producers)for kk in range(steppi))                     
                    

                    Parallel(n_jobs=-1)(delayed(plot3d2)(data_use1[0,kk + 3*steppi,:,:,:][:, :, ::-1],nx,ny,nz,kk,dt,
                    MAXZ,'unie_gas','gas_sat',max((data_use1[0,kk + 3*steppi,:,:,:][:, :, ::-1]).ravel()),
                    min((data_use1[0,kk + 3*steppi,:,:,:][:, :, ::-1]).ravel()),injectors,producers)for kk in range(steppi)) 
            ...
            if make_csv:
                ...
                if nz ==1:
                    ...
                    data_use1[:,2*steppi:3*steppi,:,:],data_use1[:,3*steppi:4*steppi,:,:],MAXZ,0,1e1,DX,steppi,\
                                    pini_alt,SWI,SWR,UW,BW,DZ,rwell,skin,UO,BO,UG,BG,\
                                pwf_producer,dt,N_inj,N_pr,nz,NecessaryI,NecessaryP,SWOW,SWOG,PB)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        sys.exit(
            'Provide Yaml scenario! python NVRS/simulator.py Run.yaml')
