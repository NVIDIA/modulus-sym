...
def eqvor_ADI_Temp(imax,jmax,dx,dy,vorn,dt,ux,vy,amu):
    ...
    for j  in range(1,jmax-1):
        for i in range(1,imax-1):
            ...
            sp = 1+epy 
            sm = 1-epy
            Dx = .5*(.5*cy*sp+d_y)*vorn[i,j-1]+(1-d_y-.5*cy*epy)*vorn[i,j]+.5*\
            (-.5*cy*sm+d_y)*vorn[i,j+1]
            d[i]=Dx
    ...
    for i  in range( 1,imax-1):
        for j  in range(1,jmax-1):
            ...
            sp = 1+epx 
            sm = 1-epx
            Dy = .5*(.5*cx*sp+d_x)*vor[i-1,j]+(1-d_x-.5*cx*epx)*vor[i,j]\
                +.5*(-.5*cx*sm+d_x)*vor[i+1,j]
            d[j]=Dy
        for j in range(jmax):
            cy = vy[i,j]*dt/dx;
            epy = np.sign(vy[i,j])
...
def eqvor_ADI_upwind(imax,jmax,dx,dy,vorn,dt,ux,vy,amu):
    ...
    for i  in range(1,imax-1):
        for j  in range(1,jmax-1):
            ...
            sp = 1+epx
            sm = 1-epx
            Dy = .5*(.5*cx*sp+d_x)*vor[i-1,j]+(1-d_x-.5*cx*epx)*vor[i,j]+.5*\
                (-.5*cx*sm+d_x)*vor[i+1,j]
            d[j]=Dy
...
RaC = rho0*beta_C*delC*g*k*H/(phi*D*mu)        # Ra for stream function (solutal)
RaT = rho0*beta_T*delT*g*k*H/(alpha*mu)        # Ra for stream function (temperature)
    ...
    for ii in range(2000):
        ...
        for i in range(1,im1):
            for j in range(1,jm1):
                strfunnew = 1/2/(kx[i,j]+kz[i,j]*beta**2)*(kx[i,j]*(strfun[i-1,j]\
                    + strfun[i+1,j])+beta**2*kz[i,j]*((strfun[i,j-1]+strfun[i,j+1]))\
...
frames[0].save(name_j,format='GIF',\
