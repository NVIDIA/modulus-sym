...
def Plot_petrophysical(permmean,poroo,nx,ny,nz,Low_K,High_K,Low_P,High_P,injectors,producers):
    ...
    plt.close()
    plt.clf()
def Plot_mean(permbest,permmean,iniperm,nx,ny,nz,Low_K,High_K,True_perm,injectors,producers): 
...
def Gassmann(PORO, Pr, SO,nx,ny,nz):
    ...
    KQuartz = 37E9  # Pa
    KFeldspar = 37.5E9  # Pa
    KMatrix = cp.ones((nx, ny)) * \
        (SQuartz * KQuartz + SFeldspar * KFeldspar) / 2 + \
    ...
    for i in range(nz):
        ...
        B0 = 0.972 + 0.00038 * \
            (2.4*RG*cp.sqrt(G/rho0) + T + 17.8)**(1.175)
        # pseudo density
        ...
        rhoOil = 1000*(rho0 + (0.00277*P - 1.71E-7*P**3) *
                       (rhoG - 1.15)**2 + P*3.49E-4) / \
            (0.972 + 3.81E-4*(T + 17.78)**1.175)  # kg/m3
...
def Plot_performance(trueF,nx,ny,nz,namet,itt,dt,MAXZ,steppi,injectors,producers):
...
def Plot_performance2(trueF,nx,ny,nz,namet,itt,dt,MAXZ,steppi,injectors,producers):
...
def Peaceman_well(inn,ooutp,oouts,MAXZ,mazw,s1,DX,steppi,\
    ...
    for kk in range(steppi):
        ...
        if nz ==1:
            ...
            right = np.log(RE/np.tile(NecessaryI[:,0], int(Injector_location.size/NecessaryI[:,0].size)))\
            + np.tile(NecessaryI[:,1], int(Injector_location.size/NecessaryI[:,1].size))
        temp = ((up/down)*right)
        #temp[temp ==-inf] = 0
        Pwf = p_inj + temp
        Pwf = np.abs(Pwf)
        ...
        if nz ==1:
            ...
            drawdown = average_pressure - np.tile(NecessaryP[:,2], int(producer_location.size/NecessaryP[:,1].size))
...
def Peaceman_well2(inn,ooutp,oouts,ooutsoil,outg,MAXZ,mazw,s1,DX,steppi,\
...
def Upstream_3PHASE(nx,ny,nz,S,Soil,UW,UO,UG,BW,BO,BG,RS,SWI,SWR,Vol,qinn,qinnoil,V,Tt,porosity):
    ...
    for t in range(Nts):
        ...
        mw,mo,mg,_,_,_ = RelPerm3(S,Soil, UW, UO, UG, BW, BO, BG, SWI, SWR, nx, ny, nz)
...
def RelPerm3(Sa,Sg, UW, UO, UG, BW, BO, BG, SWI, SWR, nx, ny, nz):
    ...
    return cp.reshape(Mw, (-1, 1), 'F'), cp.reshape(Mo, (-1, 1), 'F'), cp.reshape(Mg, (-1, 1), 'F'), \
           cp.reshape(dMw, (-1, 1), 'F'), cp.reshape(dMo, (-1, 1), 'F'), cp.reshape(dMg, (-1, 1), 'F')


    
def NewtRaph(nx,ny,nz,porosity,Vol,S,V,qinn,Tt,UW,UO,SWI,SWR,method2,BW,BO):
    """
...
def NewtRaph2(nx,ny,nz,porosity,Vol,S,Soil,V,qinn,qinnoil,Tt,UW,UO,UG,SWI,SWR,method2,BW,BO,BG,RS):
    ...
    while conv==0:
        ...
        while I < 2**IT:
            ...
            while (dsn > 0.01) and (it < 5):
                ...
                Mw,Mo,Mg,dMw,dMo,dMg = RelPerm3(S,Soil, UW, UO, UG, BW, BO, BG, SWI, SWR, nx, ny, nz)
                df=cp.divide(dMw,(Mw+Mo+ Mg + RS*Mo))-cp.multiply(cp.divide(Mw,((Mw+Mo+Mg +RS*Mo)**(2))),(dMw+dMo+dMg))  #dMw/(Mw + Mo)-Mw/(Mw+Mo)**(2)*(dMw+dMo)
                dfoil=cp.divide(dMg,(Mw+Mo+Mg +RS*Mo))-cp.multiply(cp.divide(Mg,((Mw+Mo+Mg +RS*Mo)**(2))),(dMw+dMo+dMg))
                df = cp.ravel(df)
                dfoil = cp.ravel(dfoil)
                dG=sparse.eye(N,dtype=cp.float32)-B@spdiags(df,0,N,N,format='csr')
                dGoil=sparse.eye(N,dtype=cp.float32)-Boil@spdiags(dfoil,0,N,N,format='csr')
                dG.data = cp.nan_to_num(dG.data, copy=True, nan = 0.0)
                ...
                if method2 == 1: #GMRES
                    ...
                    dsoil,exitCodeoil = gmres(-dGoil, Goil,tol=1e-6, atol=0, restart=20, maxiter=100,M=Moil)  
                    ...
                    M_xoil = lambda x: M2oil.solve(x)
                    Moil = LinearOperator((nx*ny*nz,nx*ny*nz), M_xoil)                      
                    dsoil,exitCodeoil = cg(-dGoil, Goil,tol=1e-6, atol=0, maxiter=100,M=Moil)
...
def Reservoir_Simulator(Kuse,porosity,quse,quse_water,nx,ny,nz,factorr,max_t,\
 Dx,Dy,Dz,BO,BW,CFL,timmee,MAXZ,PB,PATM,CFO,IWSw,method,steppi,SWI,SWR,UW,UO,\
     step2,pini_alt):
        ...
        if CFL == 1:
            ...
            for ts in range(step2):
                S = NewtRaph(nx,ny,nz,porosity,Vol,S,V,quse_water,\
                           St/float(step2),UW,UO,SWI,SWR,method,BW,BO)            
...
#         FINITE VOLUME RESERVOIR SIMULATOR
##############################################################################
def Reservoir_Simulator2(Kuse,porosity,quse,quse_water,quse_oil,nx,ny,nz,factorr,max_t,\
 Dx,Dy,Dz,BO,BW,BG,RS,CFL,timmee,MAXZ,PB,PATM,CFO,IWSw,IWSo,method,steppi,SWI,SWR,UW,UO,UG,\
     step2,pini_alt):
    ...
    for t in range(tc2.shape[0]-1):
        ...
        if CFL == 1:
            S,Soil = Upstream_3PHASE(nx,ny,nz,S,Soil,UW,UO,UG,BW,BO,BG,RS,SWI,SWR,Vol,quse_water,quse_oil,V,dt,porosity)
        else:
            for ts in range(step2):

                S,Soil = NewtRaph2(nx,ny,nz,porosity,Vol,S,Soil,V,quse_water,quse_oil,\
                           St/float(step2),UW,UO,UG,SWI,SWR,method,BW,BO,BG,RS)            
...
def plot3d2static(arr_3d,nx,ny,nz,namet,titti,maxii,minii,injectors,producers):
    ...
    for mm in range(n_inj):
        ...
        ax.text(x_line_end, y_line_end, z_line_end, discrip, color='black',weight='bold', fontsize=16)
    ...
    for mm in range(n_prod):
        ...
        ax.text(x_line_end, y_line_end, z_line_end, discrip, color='r',weight='bold', fontsize=16)        
    ...
    #plt.show()
    plt.clf()
def plot3d2(arr_3d,nx,ny,nz,itt,dt,MAXZ,namet,titti,maxii,minii,injectors,producers):
