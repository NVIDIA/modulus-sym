...
def inference_single(ini,inip,nx,ny,nz,max_t,Dx,Dy,Dz,BO,BW,CFL,timmee,MAXZ,\
                     factorr,steppi,\
                   aay,bby,mpor,hpor,dt,IWSw,PB,PATM,CFO,method,\
                    SWI,SWR,UW,UO,step2,pini_alt,INJ,PROD,opennI,opennP):
    ...
    for kk in range(Ne):
        ...
        A_out = Reservoir_Simulator(a,at2,quse.ravel(),\
                quse_water.ravel(),nx,ny,nz,factorr,max_t,\
                    ...
                    IWSw,method,steppi,SWI,SWR,UW,UO,step2,pini_alt)
        if nz ==1:  
...
def Select_TI(oldfolder,ressimmaster,N_small,nx,ny,nz,True_data,Low_K,High_K):
        ...
        with executor:
            Parallel(n_jobs=8,backend='loky', verbose=50)(delayed(
            Simulator)(plan,ini_ensemble1[:,kk],ensemblep[:,kk],'TI' + str(kk))for kk in range(Ne) )
            executor.shutdown(wait=False)
        # free up memory
        #executor.shutdown(wait=True)
        elapsed_time_secs = time.time() - start_time_peaceman
        msg = "Simulation of " + str(Ne) + " ensemble memebers took " + str(timedelta(seconds=round(elapsed_time_secs))) + "seconds"
        print(msg)
       ...
       # kti2=
        ...
        with executor:
            Parallel(n_jobs=8,backend='loky', verbose=50)(delayed(
            Simulator)(plan,ini_ensemble2[:,kk],ensemblep[:,kk],'TI' + str(kk))for kk in range(Ne) )
            executor.shutdown(wait=False)
        # free up memory
        #executor.shutdown(wait=True)
        elapsed_time_secs = time.time() - start_time_peaceman
        ...
        with executor:
            Parallel(n_jobs=8,backend='loky', verbose=50)(delayed(
            Simulator)(plan,ini_ensemble3[:,kk],ensemblep[:,kk],'TI' + str(kk))for kk in range(Ne) )
            executor.shutdown(wait=False)
        #executor.shutdown(wait=True)        
        elapsed_time_secs = time.time() - start_time_peaceman
        msg = "Simulation of " + str(Ne) + " ensemble memebers took " + str(timedelta(seconds=round(elapsed_time_secs))) + "seconds"
        ...
        with executor:
            Parallel(n_jobs=8,backend='loky', verbose=50)(delayed(
            Simulator)(plan,ini_ensemble4[:,kk],ensemblep[:,kk],'TI' + str(kk))for kk in range(Ne) )
            executor.shutdown(wait=False)
        #executor.shutdown(wait=True)        
        elapsed_time_secs = time.time() - start_time_peaceman
        msg = "Simulation of " + str(Ne) + " ensemble memebers took " + str(timedelta(seconds=round(elapsed_time_secs))) + "seconds"
        ...
        with executor:
            Parallel(n_jobs=8,backend='loky', verbose=50)(delayed(
            Simulator)(plan,ini_ensemble5[:,kk],ensemblep[:,kk],'TI' + str(kk))for kk in range(Ne) )
            executor.shutdown(wait=False)
        #executor.shutdown(wait=True)        
        elapsed_time_secs = time.time() - start_time_peaceman
        msg = "Simulation of " + str(Ne) + " ensemble memebers took " + str(timedelta(seconds=round(elapsed_time_secs))) + "seconds"
        ...
        with executor:
            Parallel(n_jobs=8,backend='loky', verbose=50)(delayed(
            Simulator)(plan,ini_ensembleG[:,kk],EnsemblepG[:,kk],'TI' + str(kk))for kk in range(Ne) )
            executor.shutdown(wait=False)
        #executor.shutdown(wait=True)        
        elapsed_time_secs = time.time() - start_time_peaceman
        msg = "Simulation of " + str(Ne) + " ensemble memebers took " + str(timedelta(seconds=round(elapsed_time_secs))) + "seconds"
...
def inference_single2(ini,inip,nx,ny,nz,max_t,Dx,Dy,Dz,BO,BW,BG,RS,CFL,timmee,MAXZ,\
                     factorr,steppi,\
                   aay,bby,mpor,hpor,dt,IWSw,IWSg,PB,PATM,CFO,method,\
                    SWI,SWR,UW,UO,UG,step2,pini_alt,INJ,PROD,opennI,opennP):
    ...
    for kk in range(Ne):
        ...
        #print(A)
        A_out = Reservoir_Simulator2(a,at2,quse.ravel(),\
                quse_water.ravel(),quse_oil.ravel(),nx,ny,nz,factorr,max_t,\
                    Dx,Dy,Dz,BO,BW,BG,RS,CFL,timmee,MAXZ,PB,PATM,CFO,\
                    IWSw,IWSg,method,steppi,SWI,SWR,UW,UO,UG,step2,pini_alt)
        if nz ==1:  
            ct1[0,:,:] =  at1[:,:,0] # permeability
            ct1[1,:,:] = quse1[:,:,0]#/UIR # Overall f
            ct1[2,:,:] = Ainj[:,:,0]#/UIR# f for water injection
...
def Simulator(plan,ini,inip,path_out):        
    ...
    Pressz = Pressz/maxii
    plot3d2static(Pressz,nx,ny,nz,'Permeability','Permeability',maxii,minii,injectors,producers)
    os.chdir(oldfolder)         

    start_time_plots1 = time.time()
    
    if N_components ==2:
        X_data1, data_use1 = inference_single(ini,inip,nx,ny,nz,max_t,DX,DY,DZ,BO,BW,CFL\
                                             ,timmee,MAXZ,factorr,steppi,\
                           aay,bby,mpor,hpor,dt,IWSw,PB,PATM,CFO,method,\
                            SWI,SWR,UW,UO,step2,pini_alt,injectors,producers,opennI,opennP)
        os.chdir(path_out)
        sio.savemat('UNRST.mat', {'permeability':X_data1[0,0,:,:][:,:,None],\
        'porosity':X_data1[0,3,:,:][:,:,None],'Pressure': data_use1[0,:steppi,:,:][:,:,None],\
       'Water_saturation':data_use1[0,steppi:,:,:][:,:,None],\
         'Oil_saturation': 1- data_use1[0,steppi:,:,:][:,:,None]},do_compression=True)
        os.chdir(oldfolder) 
    else:
        X_data1, data_use1 = inference_single2(ini,inip,nx,ny,nz,max_t,DX,DY,DZ,BO,BW,BG,RS,CFL\
                                             ,timmee,MAXZ,factorr,steppi,\
                           aay,bby,mpor,hpor,dt,IWSw,IWSg,PB,PATM,CFO,method,\
                            SWI,SWR,UW,UO,UG,step2,pini_alt,injectors,producers,opennI,opennP) 
        os.chdir(path_out)
        sio.savemat('UNRST.mat', {'permeability':X_data1[0,0,:,:,:],\
        'porosity':X_data1[0,3,:,:,:],'Pressure': data_use1[0,:steppi,:,:,:],\
       'Water_saturation':data_use1[0,steppi:2*steppi,:,:,:],\
         'Oil_saturation': data_use1[0,2*steppi:3*steppi,:,:,:],\
        'Gas_saturation': data_use1[0,3*steppi:,:,:,:]},do_compression=True)
        os.chdir(oldfolder)                 
            
    elapsed_time_secs = time.time() - start_time_plots1
    msg = "Reservoir simulation  took: %s secs (Wall clock time)" % timedelta(seconds=round(elapsed_time_secs))
    ...
    if N_components ==2:
        if make_animation:
            ...
            if nz ==1: 
                for kk in range(steppi):        
                    ...
                    Plot_performance(data_use1[0,:,:,:],nx,ny,nz,'new',kk,dt,MAXZ,steppi,injectors,producers)
                    ...
                    minii = min(Pressz.ravel())
                    Pressz = Pressz/maxii
                    plot3d2(Pressz,nx,ny,nz,kk,dt,MAXZ,'unie_pressure','Pressure',maxii,minii,injectors,producers)
                    
                    watsz = data_use1[0,kk + steppi,:,:][:,:,None]
                    maxii = max(watsz.ravel())
                    minii = min(watsz.ravel())
                    plot3d2(watsz,nx,ny,nz,kk,dt,MAXZ,'unie_water','water_sat',maxii,minii,injectors,producers) 
                    
                    
                    oilsz = (( 1- data_use1[0,kk + steppi,:,:][:,:,None]))
                    maxii = max(oilsz.ravel())
                    minii = min(oilsz.ravel())
                    plot3d2(oilsz,nx,ny,nz,kk,dt,MAXZ,'unie_oil','oil_sat',maxii,minii,injectors,producers)      
                ...
                frames1[0].save('Evolution2.gif',format='GIF',\
            ...
            msg = "pressure and saturation plots took: %s secs (Wall clock time)" % timedelta(seconds=round(elapsed_time_secs))
        #print(msg)
        ...
        if make_csv:
            ...
            if nz ==1:
                seeTrue = Peaceman_well(X_data1,data_use1[:,:steppi,:,:],\
                                        data_use1[:,steppi:,:,:],MAXZ,0,1e1,DX,steppi,\
                                pini_alt,SWI,SWR,UW,BW,DZ,UO,BO,\
                            dt,N_inj,N_pr,nz,NecessaryI,NecessaryP)
            else:
                seeTrue = Peaceman_well(X_data1,data_use1[:,:steppi,:,:,:],\
                                        data_use1[:,steppi:,:,:,:],MAXZ,0,1e1,DX,steppi,\
                                pini_alt,SWI,SWR,UW,BW,DZ,UO,BO,\
            ...
            os.chdir(oldfolder)
            elapsed_time_secs = time.time() - start_time_peaceman
            msg = "Well rates and pressure computation took: %s secs (Wall clock time)" % timedelta(seconds=round(elapsed_time_secs))
        ...
        if make_animation:
            ...
            if nz ==1:  
                for kk in range(steppi):   
                    ...
                    Plot_performance2(data_use1[0,:,:,:],nx,ny,nz,'new',kk,dt,MAXZ,steppi,injectors,producers)
                    ...
                    plot3d2(gasz,nx,ny,nz,kk,dt,MAXZ,'unie_gas','gas_sat',maxii,minii,injectors,producers)      
                ...
                frames[0].save('Evolution_gas_3D.gif',format='GIF',\
                                append_images=frames[1:],save_all=True,duration=500,loop=0)                    
        ...
        if make_csv:
            ...
            if nz ==1:
                seeTrue = Peaceman_well2(X_data1,data_use1[:,:steppi,:,:],\
                                        data_use1[:,steppi:2*steppi,:,:],\
                data_use1[:,2*steppi:3*steppi,:,:],data_use1[:,3*steppi:4*steppi,:,:],MAXZ,0,1e1,DX,steppi,\
                                pini_alt,SWI,SWR,UW,BW,DZ,UO,BO,UG,BG,\
                            dt,N_inj,N_pr,nz,NecessaryI,NecessaryP)
...
if not os.path.exists('TRUE'):
    ...
    elapsed_time_secs = time.time() - start_time_peaceman
    msg = "Simulation took " + str(timedelta(seconds=round(elapsed_time_secs))) + "seconds"
    print(msg)
    ...
    if Geostats==1:         
        if afresh==1:
            ...
            if Ne <=2000:
                ...
                mat = sio.loadmat(filename)
                ini_ensemblef=mat['Z']
                index = np.random.choice(ini_ensemblef.shape[1], Ne, \
                replace=False) 
        ...
        if afresh==1:
            ini_ensemblef = initial_ensemble_gaussian(nx,ny,nz,(N_ens),\
                                Low_K,High_K) 
...
post-processing\n')
    ...
    while (snn<1):        
        ...
        with executor:
            Parallel(n_jobs=8,backend='loky', verbose=50)(delayed(
            Simulator)(plan,ensemble[:,kk],ensemblep[:,kk],'Realisation' + str(kk))for kk in range(Ne) )
            executor.shutdown(wait=False)
        #executor.shutdown(wait=True)        
        simDatafinal,predMatrix = RSM_extract (Ne,'Realisation',oldfolder,scalei,scalei2)
    ...
    with executor: 
        Parallel(n_jobs=8,backend='loky', verbose=50)(delayed(
        Simulator)(plan,all_ensemble[:,kk],all_ensemblep[:,kk],'Realisation' + str(kk))for kk in range(Ne) )
        executor.shutdown(wait=False)
    simDatafinala,predMatrixa = RSM_extract (Ne,'Realisation',oldfolder,scalei,scalei2)    
    ...
    simDatafinal,predMatrix = RSM_extract1 (1,'HM_RESULTS/ADAPT_REKI',oldfolder,scalei,scalei2) 
    ...
    Plot_petrophysical(controlbest,controlbestp,nx,ny,nz,Low_K1,High_K1,Low_P,High_P,injectors,producers)
    os.chdir(oldfolder)
    Plot_RSM_percentile_model(predMatrix[0],True_mat,'HM_RESULTS/ADAPT_REKI/Compare.png')
    yycheck =  predMatrix       
    ...
    print('RMSE  = : ' \
        + str(cc) )
    os.chdir('HM_RESULTS')
    Plot_mean(controlbest,yes_mean_k,meanini,nx,ny,nz,Low_K1,High_K1,True_K,injectors,producers) 
    ...
    simDatafinal,predMatrix = RSM_extract1 (1,'HM_RESULTS/BEST_RESERVOIR_MODEL',oldfolder,scalei,scalei2)            
    os.chdir('HM_RESULTS/BEST_RESERVOIR_MODEL')
    Plot_petrophysical(yes_best_k,yes_best_p,nx,ny,nz,Low_K1,High_K1,Low_P,High_P,injectors,producers)
    os.chdir(oldfolder)
    Plot_RSM_percentile_model(predMatrix[0],True_mat,'HM_RESULTS/BEST_RESERVOIR_MODEL/Compare.png')        
    ...
    simDatafinal,predMatrix = RSM_extract1(1,'HM_RESULTS/MEAN_RESERVOIR_MODEL',oldfolder,scalei,scalei2)            
    os.chdir('HM_RESULTS/MEAN_RESERVOIR_MODEL')
    Plot_petrophysical(yes_mean_k,yes_mean_p,nx,ny,nz,Low_K1,High_K1,Low_P,High_P,injectors,producers)
    os.chdir(oldfolder)
    Plot_RSM_percentile_model(predMatrix[0],True_mat,'HM_RESULTS/MEAN_RESERVOIR_MODEL/Compare.png')  
    ...
    with executor:
        Parallel(n_jobs=8,backend='loky', verbose=50)(delayed(
        Simulator)(plan2,ensembleout1[:,kk],ensembleoutp1[:,kk],'HM_RESULTS/Percentile' + str(kk))for kk in range(ensembleout1.shape[1]) )
        executor.shutdown(wait=False)
    #executor.shutdown(wait=True)
    simDatafinal,yzout = RSM_extract (ensembleout1.shape[1],'HM_RESULTS/Percentile',oldfolder,scalei,scalei2)   
