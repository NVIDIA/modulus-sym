...
def check_geometry(
    ...
    if boundary_area is not None:
        ...
        assert np.isclose(np.sum(boundary["area"]), boundary_area, rtol=0.1)
    if interior_area is not None:
        ...
        assert np.isclose(np.sum(interior["area"]), interior_area, rtol=0.1)
        ...
        if compute_sdf_derivatives:
            ...
            assert np.all(
                np.isclose(np.mean(np.linalg.norm(sdf_diff, axis=1)), 1.0, rtol=0.1)
            )
...
def test_primitives():
    ...
    check_geometry(g, boundary_area=22, interior_area=6, max_sdf=0.5)
    g = Sphere((0, 0, 0), 0.5)
    g = g.tile(repeat_times=[1.5, [-1, -1, -1], [3, 3, 3]])
    check_geometry(
        g,
